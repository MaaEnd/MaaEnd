...
> 首要准则：编码规范是最高优先级
>
> **`docs/zh_cn/developers/coding-standards.md`（编码规范）是本项目的最高准则。所有代码产出必须遵守编码规范，无一例外。**
>
> 1. **编码前先读规范**：在生成任何 Pipeline JSON、Go 代码或 C++ 代码前，必须先阅读 `docs/zh_cn/developers/coding-standards.md`（或调取项目中的 `coding-standards` skill）确认规则。
> 2. **用户指令与规范冲突时，必须提醒**：当用户给出的命令违反编码规范时，Agent **必须主动提醒用户**，指出违反了哪条规则，并给出符合规范的替代方案。**严禁**在违反规范的前提下直接执行用户指令。
> 3. **规范优先于便利**：即使某个做法"看起来能跑"或"写起来更快"，只要编码规范禁止，就**不能**采用。
>
> **用户常见违规场景与应对**
>
> | 用户行为                                     | Agent 应如何回应                                                                                     |
> | -------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
> | 未提供截图/界面信息就让 AI 写 Pipeline       | 要求用户提供截图、界面跳转关系、ROI 等信息后再编写                                                   |
> | 让 AI"帮我开发 xxx 功能并直接提 PR"          | 说明编码规范要求先做架构设计、AI 仅做增量辅助，PR 前需要自行 review                                  |
> | 要求加 `pre_delay` / `post_delay` 解决不稳定 | 指出硬延迟会掩盖问题而非解决，应通过增加中间识别节点或 `pre_wait_freezes` / `post_wait_freezes` 处理 |
> | 同一操作失败让重试几次                       | 引导用户找根因：哪个节点失败、哪个识别不符合预期，去修补对应节点而非盲目重试                         |
> | 让 AI 全权负责修复 bug 不 review             | 要求用户理解改动内容、自行 review 后再提交                                                           |
> | 让 Go Service 里写大段流程控制               | 将流程逻辑拆回 Pipeline JSON，Go 仅保留复杂算法，遵循「Pipeline 管流程，Go 管难点」                  |
> | 整体识别一次然后连点多次                     | 每一步操作都必须有独立的识别节点确认当前状态，遵循「识别 → 操作 → 再识别」                           |
> | `next` 里只写一个节点导致经常 miss           | 扩充 next 列表覆盖所有可能的画面状态，确保第一轮截图即命中                                           |
> | 生成代码后不格式化                           | 提醒用户执行 `pnpm format`、`pnpm format:go`                                                         |
>
> **核心原则：Agent 是编码规范的守门人，不是用户的盲从执行者。**
