...
description: "向 routes.json 添加环境监测（EnvironmentMonitoring）新观察点条目。使用时：新增 zmdmap / kite_station_i18n 观察点路线配置、适配新版本的环境监测任务、补全缺失的 EnterMap、MapPath 或 NavMeshTarget 数据。会自动检测缺失任务，逐字段询问路线数据后写入 ROUTE_CONFIG。"
...
`Name` 从 `tools/pipeline-generate/data/kite_station_i18n.json` 自动提取，**不需要询问用户**：
...
| `MapTarget`                    | ✓    | 目标矩形 `[x, y, w, h]`，720p 小地图坐标，用于传送落点复核                                                                                  |
| `MapPath` / `NavMeshTarget`    | ✓    | 二选一。`MapPath` 是手录路径 `[[x1, y1], ...]`；`NavMeshTarget` 是 NAVMESH 目标点 `[x, y]`，适合不依赖交互/过图/机关的普通可达路线          |
...
- 否则：运行 [check_missing.mjs](./check_missing.mjs) 自动检测缺失条目。检测逻辑：
    - 从 `kite_station_i18n.json` 提取所有 mission 的 `name["zh-CN"]`（作为 Name）
...
4. 询问寻路方式：`MapPath` 或 `NavMeshTarget`
    - 选择 `MapPath` 时，继续询问路径（格式 `[[x1,y1],[x2,y2],...]`）
    - 选择 `NavMeshTarget` 时，继续询问目标点（格式 `[x,y]`）
...
- 4 空格缩进，`MapPath` 每个坐标对单独一行；`NavMeshTarget` 写成 `[x, y]`
- `MapPath` 与 `NavMeshTarget` 必须且只能写一个，不要同时写入
- `CameraMaxHit` 仅当值非默认（≠ 2）时才写入
- `NoEnsureInitialMovementState` 仅用于 `MapPath`，`NavMeshTarget` 条目不要写入
...
- 坐标必须基于 720p 小地图（`MapTarget`、`MapPath`、`NavMeshTarget`）。
- `NavMeshTarget` 最终会生成 `MapNavigateAction` 的 `{ "action": "NAVMESH", "target": [...] }` 语义点；它只适合普通可达路线，不处理交互、过图、机关、战斗等特殊段。
