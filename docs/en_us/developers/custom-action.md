...
It reads the recognizer config from `custom_action_param.recognition`, overrides its managed OCR node (`__RecoDetailFocusOCR`), runs recognition actively, extracts OCR `best.text`, and renders a Focus message using a template.

- **Parameters (`custom_action_param`)**

    - You may pass a JSON object, which is serialized to a string by the framework and passed to Go.
    - Field descriptions:
        - `recognition: object`: A complete MaaFramework recognizer configuration (required). This object is directly used to override `__RecoDetailFocusOCR` and run recognition.
        - `text?: string`: Display template (optional). If omitted, the default template is used: `text={text}`.

- **Supported placeholders**

    - `{text}`: OCR `best.text` from this action's recognition run.
    - `{node}`: Current node name (`CurrentTaskName`).
    - `{hit}`: Whether this recognition run is hit (`true/false`).
...
- **Notes**
    ...
    - `recognition` must be a valid MaaFramework recognizer object, otherwise the action fails.
    - This action runs OCR actively and does not depend on existing `RecognitionDetail` of the current node.
