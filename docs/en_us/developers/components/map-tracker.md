...
- `rotation_slower_threshold`: A real number in the range $(0, 180)$, default `30.0`. Rotation uses the slower speed when the directional angle deviation is below this value.
- `rotation_faster_threshold`: A real number in the range $(0, 180]$, default `60.0`, and must be greater than `rotation_slower_threshold`. Rotation uses the full adaptive speed when the directional angle deviation reaches this value, with smooth interpolation between the two thresholds.
