...
All three route-making modes use **`Select Project Node`** as their primary import entry. The path editor loads `MapNavigateAction` nodes for continued editing. When a node has at least two coordinates on the same basemap, A\* imports them as waypoints and immediately plans every consecutive leg; a lone coordinate remains a copyable target hint. Assert mode lists `MapLocateAssertLocation` nodes by default and can switch to reference routes. The selector searches by resource path, node name, or zone, so files containing several Pipeline nodes are not imported ambiguously.
