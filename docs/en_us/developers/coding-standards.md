...
Write full text in `expected`, not fragments. Multilingual handling goes through the i18n toolchain. For fragments or hand-written regex, use `// @i18n-skip`. See [Tools & debugging — OCR & i18n](./tools-and-debug.md#ocr--i18n).

### Color matching: prefer HSV / grayscale

Different GPUs render slightly differently; raw RGB is unstable across devices. See [Tools & debugging — color matching](./tools-and-debug.md#color-matching-prefer-hsv).
