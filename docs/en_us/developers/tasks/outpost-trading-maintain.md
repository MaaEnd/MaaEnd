...
- Limit 0 (cannot afford even one): the sliding node's next is overridden to `OutpostTradingAidQuotaExhausted`, whose `skip` operation marks the item as **attempted at the current location** (added to `Attempted[location]`, skipped during selection), and the sell loop tries cheaper items; the stock bill reserve is per-location, so later outposts with sufficient reserve still sell the item normally.
