...
`selection_data.json` contains five-language names merged by `itemId`, event-item filtering results, and each outpost's sellable `item_id`, `rarity`, and `unit_price` values. Outpost items retain only stable source order; the generator does not encode a business order. Go applies user priority and the selected strategy after common filtering.

The default Rarity Priority strategy compares:
...
- `agent/go-service/sellproduct/goods/`: goods data, first-page cell recognition, common candidate filtering, selection sessions, reserve rules, and runtime messages; its `strategy/` directory contains pure selection strategies, with `rarity.go`, `price.go`, and `stock.go` implementing rarity, unit-price, and stock priority respectively;
- `agent/go-service/sellproduct/operator/`: operator recognition, filtering, global restoration assignment, sessions, and caching, without depending on the goods subpackage;
- `agent/go-service/sellproduct/internal/selectiondata/`: load and validate deployment data shared by goods and operators;
- `agent/go-service/sellproduct/internal/ocrmatch/`: strict OCR matching shared by goods and operators;
- `agent/go-service/sellproduct/runtime.go`: implement the independent `SellProductLocationPlan` Custom Action and combine the goods and operator plans at the top level to produce the complete outpost runtime message;
- `agent/go-service/sellproduct/register.go`: aggregate registration for the goods, operator, and top-level plan components.
