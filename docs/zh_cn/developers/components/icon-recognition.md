...
| `grid_scale` | number | 否 | 自动选择 | 当前截图相对标准 720p UI 密度的缩放比例。自动模式只在已标定的 `1.0` 与 `1.25` profile 中选择；其它比例必须显式传入 |
...
| `threshold` | number | 否 | `0.85` | 通用物品命中的最低最终分数；默认送货界面的强排名兜底见下文，显式传入其它值时严格按该值判断 |
...
`grid_scale` 描述的是同样像素画布内的 UI 密度变化，而不是截图文件尺寸变化。组件会用 `1 / grid_scale` 缩放分析图像，复用标准 720p 网格先验，最后把返回坐标映射回原图；因此结果中的 `cell_box`、`item_box` 仍为传入原图坐标。未传入时，常规网格根据 ROI 内的重复结构在 `1.0` 与 `1.25` 两套已标定 profile 中选择，`rewards` 则按 ROI 内白色奖励卡片的尺寸选择。证据不足或两套 profile 分数接近时会返回错误，调用方可显式传入 `0.5..2.0` 范围内的比例。

组件不会根据 `grid_scale` 猜测、移动或扩大请求 ROI。调用方始终负责传入完全位于图片内、且完整覆盖目标格子的原生 ROI；自动比例判断只读取该 ROI 内的像素。奖励界面可以传整组奖励区域，也可以只传一个完整奖励卡片的 ROI。

阈值必须满足 `0 <= subpixel_threshold < threshold <= 1`。基础分低于 `subpixel_threshold` 时，组件认为当前候选明显不可靠，不再尝试更细的位置偏移，也不会把它放入 `matches`。基础分位于两个阈值之间时，组件会继续细化位置；通常只有最终分达到 `threshold`，并且没有被低纹理检查拒绝，结果才会返回。唯一例外是未显式改动默认 `threshold=0.85` 的送货界面：数量条和数量文字会系统性压低少数深色图标分数，因此经过细化、最终分不低于 `0.80` 且第一名领先第二名至少 `0.10` 的候选也可返回；调用方显式传入任何其它阈值时不会启用该兜底。送货界面的数量条和贵重品库的头像区域会从模板匹配遮罩中排除。调整阈值前应先检查 ROI、画面稳定性和候选分类。
...
| `rewards` | `GridType::Rewards` | 奖励界面 | `Isolate:*`、`ValuableDepot:*` | `[39,82,1205,511]` |
...
`request.candidates.item_ids` / `item_filters` 对应 Pipeline 和 Go 的同名参数。C++ 直接返回 `RecognitionResult`，不包含 MaaFramework 的 JSON 外层包装。
