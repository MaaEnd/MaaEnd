...
- 2D / 3D 视图：地图右上角可切换视图。2D 是路线编辑与规划预览的权威视图；3D 按 NMSH 的真实高度只读显示当前网格，可旋转、平移、缩放，使用 `W` / `S` 沿当前视角的水平方向前后移动、`A` / `D` 左右平移，也可适配视图并单击读取 `[u, v]` 与高度。3D 不修改作者路线，暂不绘制缺少逐点高度的规划路径；离开路径编辑时自动回到 2D。
