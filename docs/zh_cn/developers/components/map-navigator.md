...
除 `ZONE` 区域声明外，`path` 中的节点默认都是**可选路线 / 动作提示**，包括 `NAVMESH`、录制 `RUN`、`TRANSFER`、`PORTAL`、`INTERACT` 等。运行时先从区间起点直接规划到末端位置，让纯步行与滑索在整个区间内统一比较；直达规划可用时跳过中间提示及其动作，直达不可用时才按作者给出的完整路径逐点执行。这样机关和录制路径既能作为可达性兜底，也不会阻止运行时选择更优的新路线。
...
路径末端的位置始终是最终目标；但末点携带的动作也允许跳过，必须执行时仍应写 `required`。`ZONE` 只声明后续坐标和区域上下文，不是可执行路线点，始终作为结构边界保留。`required` 与 `strict` / `strict_arrival` 无关：前者声明路线与动作必须经过，后者只控制到点判定精度。
...
路线中的节点默认只在全局直达失败时作为提示和兜底动作。某个节点必须实际抵达并执行时，给它增加 `"required": true`；数组格式没有该字段的位置，因此必经点必须使用对象格式。
...
默认到点判定保留一定半径；标记严格后判定半径更小、到点更慢，但落点更精确。交互、跳台、传送、过图等动作本身已使用更严格的到点语义。严格到达不代表必经，也不会阻止全局规划跳过该节点；必须执行仍需 `required: true`。
...
> [!WARNING]
>
> 相邻路径点之间应当可以连贯走到。导航器无法穿模或理解业务机关；过图、跳板、下落、上升机关等路段可拆分为 `PORTAL` / `TRANSFER` / 业务节点作为作者路径兜底，业务上无条件要触发的节点再标 `required: true`。
...
展开前会先按显式 `required` 和 `ZONE` 声明划分区间。每个区间优先直接规划到末端位置，因此滑索候选与纯步行路线比较的是整个区间；若直达不可用，再按区间内的提示点及其原始动作逐段执行。
...
- 本例要求交互一定发生，所以写了 `required: true`；若该交互只用于全局直达失败后的路线兜底，可以省略。
...
4. **特殊动作点少而精。** `INTERACT`、`TRANSFER`、`PORTAL`、`HEADING` 只放在能为失败回退提供价值的位置；无条件需要触发时标 `required: true`。
