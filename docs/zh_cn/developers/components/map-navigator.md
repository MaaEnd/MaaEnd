...
- 滑索架图层：2D 路径编辑地图右上角的滑索架按钮显示当前安装目录中已记录的滑索架，状态会保存在浏览器中；单击紫色菱形可查看底图 / 世界坐标、导航高度、模板与 Level，相邻的测距工具与日志分析共用同一套 A / B 选择、可能中心跨度和几何连接判断。图层与路线级滑索开关相互独立，不改变预览参数、复制内容或实机试跑；记录候选可能未供电或已被运行时排除，几何可连接也不等于实际可用。
