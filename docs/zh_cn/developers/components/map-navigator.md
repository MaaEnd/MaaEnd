...
- 2D / 3D 视图：地图右上角可切换视图。2D 是路线编辑与规划预览的权威视图；3D 按 NMSH 中的真实高度只读显示当前网格，并服从 `地图图层` 中的 NAVMESH 开关。切换到 3D 后，可在左侧 `地图与定位` 中选择 `3D 视角导航`：默认的 `自由飞行` 使用右键拖动让相机原地转向，`W` / `S` 沿相机朝向移动，`A` / `D` 沿相机本地左右方向平移，`Space` 上升，`Ctrl + Space` 下降；`轨道环绕` 使用右键拖动围绕目标旋转，WASD 沿相机朝向的水平投影移动。两种模式均使用左键单击读取 `[u, v]` 与高度、中键拖动平移视角、滚轮缩放，也可适配视图；WASD 的移动速度会随观察距离缩放，近景移动更精细。3D 不修改作者路线，暂不绘制缺少逐点高度的规划路径；离开路径编辑时自动回到 2D。
...
连续链中的某一跳执行失败时，运行时会封禁失败跳并丢弃该链剩余跳数，再从角色实际位置重新规划。确认连续 3 次起滑都未离开架子时，角色先保持上索状态：新路线仍从脚下滑索起步就直接瞄准新落点，首段需要步行或重规划失败时才下索。其他异常仍先下索，等待定位重新稳定并贴回 navmesh，再从剩余路线中第一个实际可达的点重新接入。若没有可达接入点则本次导航明确失败，不会继续沿用从预期下索点生成的旧离索路径。
