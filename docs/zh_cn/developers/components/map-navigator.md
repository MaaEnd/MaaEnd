...
三个模式旁的 `更多` 均可选择 `读取剪贴板 JSON`。工具会自动识别完整 Pipeline、单个 `MapNavigateAction` / `MapLocateAssertLocation` 节点、`{"path": [...]}` 对象或纯 `path` 数组；导航路径缺少区域信息时，仍会要求为各段分配区域。剪贴板读取必须由按钮点击触发，首次使用时浏览器可能询问读取权限。
