...
`日志分析` 会提取每次 `MapNavigateAction`，区分作者路径提示、实际采用的步行规划、被滑索淘汰的步行基线和实际发射并确认落地的滑索跳。页面还会从 `PositionProvider::Capture` 提取 MapLocator 成功且非保持的坐标，绘制实测地面轨迹；滑索飞行和定位空窗会切断折线，不会用预测位置补齐。直接选择同一日志包的全部 ZIP 分包时，页面逐条解压 cpp-algo 日志，并读取同组 `record/Ziplines.json`：本次运行日志可定位的滑索架用编号圆点显示，快照中的同地图滑索架则以带深色描边的紫色菱形作为候选背景。候选点没有在前端复刻供电、楼层、可达性或成本规则，可能包含运行时排除项；实际选择仍以运行日志为准。日志没有记录滑索组合中接近上索点与离开末端滑索的完整规划折线，因此页面仍用明确标注的虚线表示端点连接估计，不将其冒充规划或实测轨迹。文件只在浏览器本地读取，日志模式不能修改路线或发起实机试跑。
