...
- 路线规划：点击 `规划当前片段`，以当前片段的第一个作者路点为离线预览起点。粉色折线是运行时展开的步行段，青色有向边是选中的滑索跳；展开「寻路诊断」可分别显示 A*、二次重寻、StringPull、端点拼接、交叉点处理、Slim、WidenCorners 与最终路线。诊断只包含完整展开最终采用的步行段，不包含失败的全局直达尝试或未选中的步行基线。预览不会改写作者路点。失败时状态栏会标明失败的作者路点，并显示起终点离网、端点不连通、指定目标面不可达或 navmesh 加载失败等运行时原始原因。
