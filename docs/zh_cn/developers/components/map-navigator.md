...
只有启用滑索时，`path` 中用于移动或可达性兜底的节点才作为**可选路线 / 动作提示**参与全局优化，包括 `NAVMESH`、录制 `RUN`、`SPRINT`、`JUMP`、`FIGHT`、`TRANSFER`、`PORTAL`、`INTERACT` 等。运行时先从区间起点直接规划到末端位置，让纯步行与滑索在整个区间内统一比较；直达规划可用时跳过中间提示及其动作，直达不可用时才按作者给出的完整路径逐点执行。这样机关和录制路径既能作为可达性兜底，也不会阻止滑索规划选择更优的新路线。

未启用滑索时不做跨作者路径点的全局比较，也不会跳过任何动作：普通点按作者顺序保留，`NAVMESH` 只逐点展开，显式 `target_tier` 仍按原规则投影。

以下节点不是可达性提示，默认不能跳过，也不需要额外填写 `required: true`：

- `ZONE`：区域上下文的结构边界；
- `HEADING`：明确的无坐标或目标朝向控制指令；
- `COLLECT`：启用行进采集扫描的业务标记，删掉后该区域不会扫描采集物；
- `DIG`：到点后派发挖掘子任务的业务动作。

确实必须抵达并执行的节点写 `"required": true`。它会结束当前规划区间，后续路径从该点重新开始：
...
启用滑索时，路径末端的位置始终是最终目标；普通末点动作仍允许跳过，必须执行时应写 `required`。`HEADING`、`COLLECT`、`DIG` 保持上述固有必执行语义；`ZONE` 只声明后续坐标和区域上下文，不是可执行路线点，始终作为结构边界保留。`required` 与 `strict` / `strict_arrival` 无关：前者声明滑索全局规划中的路线与动作必须经过，后者只控制到点判定精度。
...
启用滑索时，路线中用于移动或可达性兜底的带坐标节点，默认只在全局直达失败时作为提示和兜底动作。某个节点必须实际抵达并执行时，给它增加 `"required": true`；数组格式没有该字段的位置，因此必经点必须使用对象格式。`COLLECT`、`DIG` 是固有业务边界，默认保留；无坐标或目标朝向的 `HEADING` 是控制指令，也默认始终执行。未启用滑索时，所有作者节点均按顺序执行，`required` 不改变这一行为。

例如传送只是旧路线的可达性兜底时保持默认即可；业务上必须触发这次传送时写成：
...
默认到点判定保留一定半径；标记严格后判定半径更小、到点更慢，但落点更精确。交互、跳台、传送、过图等动作本身已使用更严格的到点语义。严格到达不代表必经；启用滑索时，它也不会阻止全局规划跳过该节点，必须执行仍需 `required: true`。未启用滑索时节点本就按作者顺序执行。
...
`HEADING` 是明确的控制指令，默认不会被滑索全局规划跳过，也不需要填写 `required: true`。它会结束前一段全局规划：前面存在移动目标时，导航器先抵达该目标，再执行朝向调整。
...
> [!NOTE]
>
> 页面的点编辑面向带坐标的路径点（`RUN / SPRINT / JUMP / FIGHT / INTERACT / PORTAL / TRANSFER / COLLECT / DIG / NAVMESH`），可为单点编辑 `required` 与 `target_tier`，并由区域信息派生 `ZONE` 声明。`HEADING` 是无坐标控制节点，不属于该编辑模型，建议在导出 `path` 后手动补充维护；`NAVMESH` 也可以在 `A* 寻路` 模式中直接复制。
...
> [!WARNING]
>
> 相邻路径点之间应当可以连贯走到。导航器无法穿模或理解业务机关；过图、跳板、下落、上升机关等路段可拆分为 `PORTAL` / `TRANSFER` / 业务节点作为作者路径兜底。启用滑索时，业务上无条件要触发的节点再标 `required: true`；未启用滑索时这些节点均按作者顺序执行。
...
只有启用滑索时，展开流程才会按显式 `required`、固有的 `HEADING` / `COLLECT` / `DIG` 业务边界和 `ZONE` 声明划分全局规划区间。每个区间优先直接规划到末端位置，因此滑索候选与纯步行路线比较的是整个区间；若直达不可用，再按区间内的提示点及其原始动作逐段执行。固有动作位于区间末端时会在抵达后保留并执行，再从这里开始下一段规划。未启用滑索时，`NAVMESH` 按作者顺序逐点展开，不进行跨点比较。
...
- 本例要求交互一定发生，所以写了 `required: true`；启用滑索且该交互只用于全局直达失败后的路线兜底时，可以省略。未启用滑索时无论是否填写都会按作者顺序执行。
...
4. **特殊动作点少而精。** `INTERACT`、`TRANSFER`、`PORTAL` 只放在能为滑索全局规划失败回退提供价值的位置；启用滑索且无条件需要触发时标 `required: true`。`HEADING`、`COLLECT`、`DIG` 写入路线即会保留，其中后两者属于任务本身的业务动作。
