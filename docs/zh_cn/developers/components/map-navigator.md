...
`日志分析` 会提取每次 `MapNavigateAction`，区分作者路径提示、实际采用的步行规划、被滑索淘汰的步行基线和实际发射并确认落地的滑索跳。页面还会从 `PositionProvider::Capture` 提取 MapLocator 成功且非保持的坐标，绘制实测地面轨迹；滑索飞行和定位空窗会切断折线，不会用预测位置补齐。直接选择同一日志包的全部 ZIP 分包时，页面逐条解压 cpp-algo 日志，并读取同组 `record/Ziplines.json`：本次运行日志可定位的滑索架用编号圆点显示，快照中的同地图滑索架则以带深色描边的紫色菱形作为候选背景。默认单击地图点位用于查看作者点、规划点、实测点或滑索架的具体信息；需要测距时，必须先开启右上角的测距工具，再依次选择 A / B 两座滑索架。点位详情和测距结果相互独立，拖动地图仍是平移。森空岛记录的是随朝向变化的角格锚点，因此地图和侧栏按运行时同款 `footprint` 算法显示可能中心的世界三维跨度范围、原始锚点距离、水平距离范围、高差和底图像素距离。高度差原样参与，不应用占地偏移；同类型、同 `level_id` 且范围下界不超过 `max_span` 时，页面标为几何上可连接。这个判定不包含供电、可达性、禁用边与成本规划，实际选择仍以运行日志为准。日志没有记录滑索组合中接近上索点与离开末端滑索的完整规划折线，因此页面仍用明确标注的虚线表示端点连接估计，不将其冒充规划或实测轨迹。文件只在浏览器本地读取，日志模式不能修改路线或发起实机试跑。
...
工具会递归扫描文件中可识别的 `path`，自动载入点数最多的一条。源数据缺少 zone 信息时，需先在页面中为各段分配区域。导入时严格校验动作语义，**未知动作直接拒绝**，不做静默降级。
