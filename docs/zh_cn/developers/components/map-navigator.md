...
2. **选择层级**：在左侧 `选择路径底图与层级` 中选中目标所在的底图 / tier。
3. **标记目标**：选择 `加 NAVMESH/选择`，在地图上点击目标位置。手工新增的路点默认使用 `NAVMESH`；只有实机轨迹录制产生的点默认使用 `RUN`。
4. **复制**：点击 `复制路径`。
...
`断言模式` 可以直接连接游戏标出当前位置参考点；路径编辑开始录制后，页面底部会持续刷新坐标、区域和 MapLocator 实测朝向（`0°` 为北、`90°` 为东）。
...
- 路径点：先选择路径底图与层级；`加 NAVMESH/选择 (1)` 手工添加 `NAVMESH` 目标，也用于选中、拖拽点；`框选工具 (2)` 用于框选多个点。实机录制得到的轨迹点保持 `RUN`。
