...
| `路线制作` | `路径编辑` | 连接游戏录制路线，编辑路径点动作 | `复制路径` → 完整 `path` |
...
- `路径编辑`：选择带有效 `custom_action_param.path` 的 `MapNavigateAction`，载入完整路径继续编辑；
