...
`日志分析` 会提取每次 `MapNavigateAction`，区分作者路径提示、实际采用的步行规划、被滑索淘汰的步行基线和实际发射并确认落地的滑索跳。日志没有记录接近上索点与离开末端滑索的完整折线，因此页面只用明确标注的虚线连接端点，不将其表示成精确轨迹。文件只在浏览器本地读取，日志模式不能修改路线或发起实机试跑。
