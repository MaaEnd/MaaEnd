...
3. **标记目标**：在地图上点击目标位置；已有 `[x, y]` 坐标时，也可粘贴到左侧任一坐标框，工具会自动填入 X / Y，再点击 `标点`。
4. **复制**：点击 `复制路径`。
...
页面顶部先区分两个工作区：`路线制作` 与只读的 `日志分析`。路线制作内部再分为三个工具：

| 工作区 | 工具 | 操作 | 导出内容 |
| ---------- | ---------- | ------------------------------------------ | ------------------------------------------------ |
| `路线制作` | `A* 寻路` | 选择底图与层级，在图上标记目标点并预览路线 | 复制 `NAVMESH` 节点，可直接放入环境监测 `NavPath` |
| `路线制作` | `断言模式` | 框选矩形区域 | 复制 `MapLocateAssertLocation` 节点，或仅复制环境监测 `NavAssert` 坐标 |
| `路线制作` | `路径编辑` | 连接游戏录制路线，编辑动作，并按运行时语义规划当前片段 | 默认复制完整 `path`；滑索模式复制完整 `custom_action_param` |
| `日志分析` | — | 本地导入 `cpp-algo/debug/maafw*.log` 或 MaaEnd `-partNN.zip`，在对应底图上复盘步行/滑索决策 | 不导出；显示成本计算、原始原因、滑索快照和实际落地记录 |

路线制作统一提供连接设置、当前位置读取、录制与实机试跑。日志分析不连接游戏；进入该工作区后，页面会隐藏连接、实时位置、试跑和路线制作页签，并暂停连接状态探测。切回路线制作时保留此前使用的模式和编辑内容。
...
即[快速上手](#快速上手)中的节点，由 `A* 寻路` 模式的 `复制路径` 导出。
...
- 路线规划：点击 `规划当前片段`，以当前片段的第一个作者路点为离线预览起点。粉色折线是运行时展开的步行段，青色有向边是选中的滑索跳；预览不会改写作者路点。
...
三个路线制作工具都以 `选择项目节点` 作为主导入入口。工具扫描当前项目 `assets` 下的 JSON / JSONC，并按当前模式只展示适用节点：

- `路径编辑`：选择带有效 `custom_action_param.path` 的 `MapNavigateAction`，载入完整路径继续编辑；节点已有 `zip: true` 时同步勾选滑索开关；
- `A* 寻路`：选择 `MapNavigateAction`；同一底图中的坐标会按原顺序载入为目标点。无论导入一个还是多个目标，都先在地图上单击一次补充手动起点，再自动规划“手动起点 → 导入点 1 → 导入点 2…”；单个目标在补起点前仍可直接复制；列表同时显示总点数与 `NAVMESH` 点数；
...
三个模式都直接提供 `读取剪贴板` 按钮。工具会自动识别完整 Pipeline、单个 `MapNavigateAction` / `MapLocateAssertLocation` 节点、`{"path": [...]}` 对象或纯 `path` 数组；导航路径缺少区域信息时，仍会要求为各段分配区域。剪贴板读取必须由按钮点击触发，首次使用时浏览器可能询问读取权限。
...
> [!NOTE]
>
> 页面的点编辑面向带坐标的路径点（`RUN / SPRINT / JUMP / FIGHT / INTERACT / PORTAL / TRANSFER / COLLECT / DIG / NAVMESH`），可为单点编辑 `required` 与 `target_tier`，也可为单个 `NAVMESH` 目标选择 `target_deck_y`，并由区域信息派生 `ZONE` 声明。`HEADING` 是无坐标控制节点，不属于该编辑模型，建议在导出 `path` 后手动补充维护；`NAVMESH` 也可以在 `A* 寻路` 模式中直接复制。
