...
| `日志分析` | 本地导入 `cpp-algo/debug/maafw*.log`，在对应底图上复盘步行/滑索决策 | 不导出；显示成本计算、原始原因和实际落地记录 |
...
`日志分析` 会提取每次 `MapNavigateAction`，区分作者路径提示、实际采用的步行规划、被滑索淘汰的步行基线和实际发射并确认落地的滑索跳。页面还会从 `PositionProvider::Capture` 提取 MapLocator 成功且非保持的坐标，绘制实测地面轨迹；滑索飞行和定位空窗会切断折线，不会用预测位置补齐。日志没有记录滑索组合中接近上索点与离开末端滑索的完整规划折线，因此页面仍用明确标注的虚线表示端点连接估计，不将其冒充规划或实测轨迹。文件只在浏览器本地读取，日志模式不能修改路线或发起实机试跑。
