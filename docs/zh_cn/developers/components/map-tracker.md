...
- `rotation_slower_threshold`: 介于 $(0, 180)$ 的实数，默认 `30.0`。方向角偏离低于该值时使用较慢的转向速度。

- `rotation_faster_threshold`: 介于 $(0, 180]$ 的实数，默认 `60.0`，且必须大于 `rotation_slower_threshold`。方向角偏离达到该值时使用完整的自适应转向速度；在两个阈值之间时平滑插值。
...
- `zoom_value`: 介于 $[0, 1]$ 的实数，默认 `0`。开始匹配之前先将大地图缩放滑块调整到该位置。若设为 `0`（默认），则表示不进行缩放滑块的调整。执行非零缩放后，节点会重新截图，并使用同一张新截图完成地图视口推断和模板匹配。
