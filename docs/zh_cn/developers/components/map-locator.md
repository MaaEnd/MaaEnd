...
| `camRot` | 摄像机朝向，0°–360°，北为零向。定位成功时由参考配对工件输出，坐标定位失败时回退到观测模型（机制见[定位原理](#定位原理)）；小地图被遮挡（`None`）的帧不输出。与 `rot`（角色朝向）无关联 |
| `camRotConf` | 摄像机朝向置信度 |
...
该节点执行的是**等待式判定**：重置追踪状态并强制全局搜索，随后以 250ms 间隔轮询（最多 60 帧，约 15 秒），直到某一帧定位成功且坐标落在矩形内为止。传送落地时区域横幅会压住小地图，这段预算用于等它散去。因此调用可能阻塞数秒，并非瞬时判定。
...
没有可选参数。断言始终强制全局搜索且只接受 `zone_id` 内的定位结果，匹配分数与 YOLO 置信度沿用定位器默认值，与 `MapNavigator` 取位时一致。
...
5. **摄像机朝向工件**：`camRot` / `camRotConf` 由 `assets/resource/model/map/cameraorientation/` 下同一批交付的三图工件输出。`preprocess.onnx` 是前处理的唯一实现：输入 118×120 观测 ROI（BGR）、zone 底图资产（BGRA，尺寸动态）与定位 `(x, y, scale)`，输出观测条带与参考条带（含参考原始 alpha）；极坐标几何、参考采样与条带域合成、采样与取整约定全部封装在图内。`polar.onnx` 消费 3 通道观测条带；`polar_with_ref.onnx` 消费 7 通道 `[obs.BGR, ref.BGR, ref.A]` 参考配对；两者输出同构的 360 bin 方位角概率分布。MapLocator 只保留三项判断：参考 alpha 缺口占比、按阈值分派（**严格大于 30%** 用 `polar.onnx`，否则用 `polar_with_ref.onnx`）、PMF 解码（角度 + 置信度）。参考分类器未加载或参考底图缺失/非 BGRA 时走 `polar.onnx`；分派后所选分类器不可用或推理失败时该帧没有 `camRot`，不做跨模型回退。坐标定位失败帧没有 `(x, y, zone)`，参考采样无从谈起；但 `preprocess.onnx` 的观测条带只由 `minimap` 决定，因此只产出观测条带、固定走 `polar.onnx` 即可给出不依赖坐标的朝向（被遮挡帧条带无效，不输出）。
