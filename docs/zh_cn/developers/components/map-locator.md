...
| `camRot` | （成功时）摄像机朝向，0°–360°，北为零向。由两个摄像机朝向模型之一输出（见[定位原理](#定位原理)），与 `rot`（角色朝向）无关联 |
...
| `rot` | （成功时）朝向偏航角 |
| `camRot` | （成功时）摄像机朝向，由两个摄像机朝向模型之一输出（见[定位原理](#定位原理)） |
| `camRotConf` | （成功时）摄像机朝向置信度 |
| `locConf` | 本次命中的置信度得分 |
| `latencyMs` | 本次计算耗时（毫秒） |
...
5. **摄像机朝向双模型分派**：`camRot` / `camRotConf` 由两个模型之一输出。观测模型 `assets/resource/model/map/cameraorientation.onnx` 只输入极坐标展开的观测条带；参考配对模型 `assets/resource/model/map/cao_ref.onnx` 同时输入观测条带、按定位结果从 zone 底图裁出的同视野参考与参考原始 alpha，共 7 通道。两者共用同一套极坐标几何与解码。定位成功且区域非 `None` 时按参考缺失占比分派：参考 alpha 在展开环 42×360 上的缺失像素（`< 255`）占比**严格大于 30%** 时用观测模型，否则用参考配对模型。参考底图资产缺失、尺寸不符或参考模型未加载时回退观测模型；所选模型不可用或推理失败时该帧没有 `camRot`，**不做跨模型回退**。
