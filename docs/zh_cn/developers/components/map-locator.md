...
| `camRot` | （成功时）摄像机朝向，0°–360°，北为零向。由摄像机朝向工件输出（见[定位原理](#定位原理)），与 `rot`（角色朝向）无关联 |
| `camRotConf` | （成功时）摄像机朝向置信度 |
...
| `rot` | （成功时）朝向偏航角 |
| `camRot` | （成功时）摄像机朝向，由摄像机朝向工件输出（见[定位原理](#定位原理)） |
| `camRotConf` | （成功时）摄像机朝向置信度 |
| `locConf` | 本次命中的置信度得分 |
| `latencyMs` | 本次计算耗时（毫秒） |
