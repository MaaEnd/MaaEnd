...
`retry_path` 使用与 `MapNavigateAction.custom_action_param.path` 相同的格式。它从主路线结束后的实际站位开始执行，应包含独立运行所需的区域声明和路径点。所有仓储和送货目标未配置时都使用自动生成的“8 米必经接近点 → 原始终点”路线，无需手工复制坐标；自动路线与主路线同源，从接近点到终点一律带 `target_deck_y`，共用实体的世界高度。这不改变 NPC 主路线仍为单个终点的规则。
