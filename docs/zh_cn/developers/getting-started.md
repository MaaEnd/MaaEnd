...
```json
{
    "icon": "resource/image/icon/item_lbmob_1_lbshamman_1_sp_1_1.png",
    "description": "![](resource/image/icon/item_char_break_stage_3_4.png)"
}
```

图片路径相对于 `assets/` 目录，因此不要包含 `assets/` 前缀，统一从 `resource/` 开始。需要显示物品识别生成的图标时，按 `resource/image/icon/<物品 ID>.png` 拼接路径，其中 `<物品 ID>` 使用 `assets/data/IconRecognition/recognition_items.json` 的顶层键。
