...
- 所有图片和坐标以 **1280×720** 为基准，下图中我们使用 **Maa Pipeline Support**，无需自己切换游戏分辨率，framework 会自动会自动改变图片尺寸。
...
如果需要等 UI 稳定，可以使用 `pre_freeze_delay` 或 `post_freeze_delay`：默认会计算匹配 ROI 部分的像素变化；例如上文中 `"post_freeze_delay": 100` 表示在 `roi` 区域 `[400, 200, 480, 320]` 内像素变化结束后，再等待 100 ms。
