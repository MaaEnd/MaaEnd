...
- `key_regex: string`：可选。从 OCR 原文提取写入 `attach.visited` 的 key（有捕获组取第 1 组，否则取整段匹配；未命中则用原文）。组件**不做**任何内置业务文案归一，规则由 Pipeline 声明。
- `blacklist_prefix: string`：可选。拼进负向黑名单：`^(?!<prefix>(?:key1|key2)<suffix>$)`。默认空串表示 key 从字符串开头匹配；若 key 可能出现在中间（例如备注名包裹原名），可传 `".*"`。
- `blacklist_suffix: string`：可选。拼进负向黑名单：`^(?!<prefix>(?:key1|key2)<suffix>$)`。默认空串即精确到末尾；若业务要容忍 key 后的 OCR 尾噪，自行传入如 `"(?:\\D.*)?"`。

行为：

1. 从 `visited_node`（或当前节点）读取 `attach.visited`。
2. 解析 `candidate` 的 key OCR（And 用 `box_index`），读取其 `expected`，按 `visited`（及可选 `blacklist_suffix`）拼负向黑名单并只覆盖 `expected`（`order_by` 等其它字段保持原样）。
3. 执行 `candidate`；未命中则失败。
4. 从命中结果取 OCR 文案；若配置了 `key_regex` 则先提取 key，再写入上述节点的 `attach.visited`，返回命中框。

候选结构、点击目标、备注优先（多项 `expected` + `order_by: Expected`，或拆成两个消费节点 + 共享 `visited_node`），以及 OCR 抖动的 key/黑名单规则，均由 Pipeline 配置。
...
- `key_regex` / `blacklist_prefix` / `blacklist_suffix` 仅供业务声明匹配策略，通用模块不会内置具体游戏文案规则。
### ScheduleRecognition
