...
二者实现均位于 `agent/go-service/common/repeataction`，用于反复执行一次内置动作或自定义动作，并在每次执行后检查等待节点；条件满足即成功，耗尽次数仍不满足则失败。
...
- 公共参数：
    ...
    - `custom_action?: string`：已注册自定义动作名（如 `AutoAltClickAction`）；与 `action` **二选一**。
