...
2、当某个子任务失败后再跑一次
_重试只能略微提高成功率，并不能根本解决问题，只会让代码变的难以维护，最后演变成，A失败了换成B试试，B失败了换成C试试，A重试3次，B重试2次，问题变的难以定位_
3、当一个节点出现死循环后加max_hit
_出现死循环一般都是识别问题、逻辑缺陷导致，盲目加maxhit只会让逻辑中断，类似在代码里盲目抛异常跳出任务，出现难以预估的后果_
...
_没有必要的go逻辑会大大增加代码复杂度，造成下一位开发者开发调试极其困难、跨平台适配十分艰难_
