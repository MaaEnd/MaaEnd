...
MaaEnd是长期维护的开源项目，非一次性用完即扔能跑就行的外包项目

### 禁止无脑使用ai进行开发

- 对着ai说“帮我开发xxx功能并pr”“帮我修这个bug并pr”
- 在关键模块中使用ai生成难以维护、无法理解的大量黑盒代码，例如使用ai进行毫无意义的过度封装、简单功能增加几千行go、C++代码。
- 在关键模块中提交自己看不懂、无法掌控的代码。

_custom代码写出来只有开发者自己能维护，当开发者自己都看不懂，别说扩展新功能了，就是修bug也没人能修。且目前市面上ai模型修改本项目bug正确率过低。_

### 推荐ai开发方式

- 学习本项目编程规范，自行进行架构设计、或参考ai架构设计，使用ai进行有目的开发，自行review生成的代码是否符合预期，最终提交pr
...
3、当一个节点出现死循环后加max_hit。
_出现死循环一般都是识别问题、逻辑缺陷导致，盲目加 `max_hit` 只会让逻辑中断，类似在代码里盲目抛异常跳出任务，出现难以预估的后果_
