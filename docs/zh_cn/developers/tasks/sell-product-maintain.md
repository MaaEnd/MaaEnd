...
2. 仅提供信用点加成；
3. 仅提供经验加成；
4. 同档加成优先沿用当前派驻；需要更换时，选择能让全局恢复方案沿用更多售卖干员的候选；
5. 全局恢复结果仍相同时，按游戏干员列表顺序稳定选择。

`selection_data.json` 为每个售卖候选保留 `bonus_tier`，避免把同档候选的稳定顺序误当成收益差异。当前派驻属于最高可用档时，Pipeline 不打开干员列表；需要更换时，Go 会逐个评估同档候选对应的全局恢复方案。
...
3. 保留数量也相同时，选择候选 `Priority` 总和更小的方案；
4. 已确认的 `location -> operator` 立即锁定，后续据点不能重复分配该干员。
...
`SellProductLogPlan` 在正式售卖前生成结构化日志和 UI 节点通知。结构化日志的 `operator_plan` 记录启用据点的售卖与恢复干员，`item_priorities` 记录应用用户覆盖后的货品顺序，`reserve_rules` 记录物品及其保留数量。UI 运行日志先显示总计划，再按游戏顺序显示每个启用据点的本地化名称、售卖/恢复干员、完整物品优先级和适用的保留数量。日志内容不包含缓存状态、稀有度或单价。

任务运行期间，UI 日志在状态确认后显示进入的据点、干员沿用或切换结果、当前货品和交易完成状态。干员已在其他据点派驻时显示被排除的候选和重新规划原因；完整扫描产生新方案时显示据点、用途和新干员。售卖干员不可用、干员扫描失败以及恢复干员不可用并跳过恢复时也会显示对应结果。任务内的 UI 提示均使用当前客户端语言。
