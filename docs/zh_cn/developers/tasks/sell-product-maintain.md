...
生成器位于 `tools/pipeline-generate/SellProduct/`。zmdmap 数据 CI 通过 `data/scripts/sell_product_data.py` 从 TableCfg 生成 `tools/pipeline-generate/data/sell_product.json`，其中只保留据点、可售物品、据点特性与干员匹配关系；MaaEnd 通过 `fetch-data.mjs` 下载该文件。`model.mjs` 统一定义据点/地区/多语言键，各 `*-data.mjs` 是对应模板的最小数据投影。
...
- 新增货品通常只需同步 zmdmap 精简数据；`sync-locales.mjs` 自动补齐五语言 `item.*` 键（中文名相同的复用旧键）。活动物品临时排除集中在 `selection-data.mjs`，原始数据移除后应清理并重新生成。
