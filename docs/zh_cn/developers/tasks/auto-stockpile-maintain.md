...
3. 再截图扫描次屏。
4. 按商品 **ID 去重合并**（同 ID 保留首屏结果；仅次屏出现的 ID 记入 `SecondPageOnlyIDs`）。
...
- 若商品 ID 在 `SecondPageOnlyIDs` 中（例如最后一排只在次屏可见），点击前会再执行一次 `AutoStockpileSwipeShelfDown`，再交给 `AutoStockpileSelectedGoodsClick` 做模板点击。
- 若首屏已有该商品，则不额外滑动。
