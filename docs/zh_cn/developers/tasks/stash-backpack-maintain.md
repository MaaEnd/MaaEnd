...
- 补充可用道具时按物品合并目标并保留原始堆叠数，每次拖动消耗一次处理机会，再从当前位置继续查找同名未满格。每页识别全部同名格，按顺序检查数量并跳过达到 `50` 的格子；同一屏幕页内已尝试的格子不再处理，即使数量 OCR 失败。识别时仅保存候选，拖动返回后才标记已尝试，避免挡住拖动前的二次识别；背包滚动后清空记录，仓库滚动和重新查找不清空。当前页没有候选时继续翻页，搜索结束仍无候选或仓库缺货时整组跳过。处理次数以初始堆叠数为上限，防止跨页反复尝试。
