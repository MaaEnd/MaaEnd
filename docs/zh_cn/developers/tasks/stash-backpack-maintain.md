...
取回验证采用计数复核：每次转移前记录仓库当前页该物品的格子数基线，转移后重新识别，格子数比基线至少少一即判定成功，仓库中存在多个同物品堆叠时依然成立。判定未移动，或转移动作本身执行失败（经 `on_error` 进入同一路径）时，停止后续取回，输出红色提示（含剩余数量），剩余目标写回存放记录。
...
- 存放以当前页目标格子数减少验证；取回以仓库当前页该物品格子数较转移前基线减少验证，不依赖背包页扫描。
