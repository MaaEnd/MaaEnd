...
3. 如果使用 `Win32` 连接，游戏已经启动，并且窗口**没有最小化**。
4. 如果使用 `ADB` 连接，`adb` 已可用，且目标模拟器 / 设备已经出现在设备列表中。
5. 当前角色已经站在你想录制路线的起点附近。
...
3. 把需要跳跃、冲刺、外力传送或过图的点改成对应动作（例如 `JUMP` / `SPRINT` / `TRANSFER` / `PORTAL`）。
4. 检查跨区前后两个点是否落在合理位置。
