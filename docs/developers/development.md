...
- 应通过扩充 `next` 覆盖过渡动画/加载中等中间状态节点、或使用pre_wait_freezes、post_wait_freezes等待画面禁止，在确认按钮可点击时再执行点击。避免对同一按钮重复点击——第二次点击可能已经作用于下一界面的其他元素，造成逻辑错误。详见 [Issue #816](https://github.com/MaaEnd/MaaEnd/issues/816)。
