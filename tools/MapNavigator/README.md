...
路径编辑页始终显示当前底图与层级对应的 navmesh 网格。点击 `规划当前片段`，会以当前区域片段的第一个作者路点为离线预览起点，将该片段的完整 `path` 交给 C++ `MapNavigateAction` 展开器。粉色折线表示运行时生成的步行段，青色有向边表示选中的滑索跳；展开 `寻路诊断` 后可分别查看 A*、二次重寻、StringPull、端点拼接、交叉点处理、Slim、WidenCorners 与最终路线。诊断只记录完整展开最终采用的步行段，不会把全局直达失败尝试或未选中的步行基线混入结果。`required`、`target_tier`、`target_deck_y` 与动作边界均使用实际运行时语义。规划失败时，状态栏会显示失败的作者路点及运行时原始原因，例如起点或终点离网、端点不连通、指定目标面不可达或 navmesh 加载失败。单独选中 `NAVMESH` 路点时，属性面板会查询该坐标的重叠可走面，可预览后选择或清除 `target_deck_y`；移动目标、切换坐标层级或改成其他动作会清除旧的目标面声明。跨区域路线按页面当前片段分别预览，避免把不同底图坐标画在一起。
