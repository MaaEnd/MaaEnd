...
左侧「连接状态」下方、页签之上的“实机试跑”面板对三个页签一律可用，跑的永远是**当前页签里那条线**：路径编辑跑编辑器路点，A\* 跑「复制配置」会给出的那串 `NAVMESH` 目标（同一处口径，两者不会漂移），断言模式没有线可跑。改线后自动重新装载，不必手动同步。
...
- `navtest_service.py`: 实机试跑会话，把当前页签里的路线交给 `MapNavigateAction` 走一遍，F3 重跑 / F4 终止。
