...
5. 单击一座滑索架选择 A 点，再单击另一座选择 B 点；地图会画出测距虚线，侧栏按运行时同款世界三维距离列出计算式、水平距离、高差、底图像素距离和几何连接判定。拖动地图仍是平移，再次单击 A 或点击「清除」可取消选择。
...
- A / B 外圈与两点间虚线：手动选择的测距对象。主结果为 `sqrt(dx² + dy² + dz²)` 世界三维距离（米）；同类型、同 `level_id` 且不超过该类型 `max_span` 时只标为“几何上可连接”。这不代表供电、可达性、禁用边与成本规划均已通过，实际能否采用仍以运行时决策为准。
