...
5. 默认单击地图上的作者路径点、规划点、实测点或滑索架，可在侧栏查看来源、序号、坐标与相应运行信息；拖动地图仍是平移。
6. 需要测量滑索架间距时，先点击地图右上角的测距工具，再依次选择 A、B 两座滑索架。地图会画出测距虚线，侧栏按运行时同款占地算法列出可能中心跨度范围、原始锚点距离、水平距离范围、高差、底图像素距离和几何连接判定。再次单击 A 或点击「清除」可取消选择；退出测距工具不会清除已有结果。
