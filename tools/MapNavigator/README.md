...
4. 只测试任意两点时，点击工具栏的 `快速测试`，再依次单击起点和终点；需要从作者路径以外的位置测试完整片段时，点击地图右上角的 `设置规划起点` 并在地图取点，再点击 `规划当前片段`。两种方式都使用 C++ 运行时路线展开逻辑；需要评估滑索时，先勾选 `启用滑索规划（桌面端）`。
