...
- 支持只读的 `日志分析` 模式：浏览器本地导入一个或多个 `cpp-algo/debug/maafw*.log`，按时间和节点选择 `MapNavigateAction`，在对应底图上对照作者提示、实际采用的步行规划、MapLocator 实测地面轨迹、被淘汰的步行基线和实际发射并落地的滑索段。
...
1. 解压 MaaEnd 日志包，切到 `日志分析` 页签。
2. 导入一个或多个 `cpp-algo/debug/maafw*.log`。
...
日志由浏览器本地读取，不会上传到后端。日志模式只允许缩放和平移，也不会向实机试跑装载路线。
