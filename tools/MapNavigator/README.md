...
页面分为两个顶层工作区：`路线制作` 内含路径编辑和断言模式，并统一提供连接、实时位置与实机试跑；`日志分析` 是纯离线只读工作区，进入后隐藏这些实机能力和路线制作页签，同时暂停连接状态探测。切回路线制作时会恢复离开前使用的页签和编辑状态。
...
`断言模式` 的“标出游戏内当前位置”按钮会临时连接游戏，连续取得 3 个有效定位帧后显示第 3 帧的坐标与朝向。底部罗盘按 MapLocator 约定显示精确角度和八方位文字：`0°` 为北、`90°` 为东，角度顺时针增加；地图参考点上的青色箭头表示同一朝向。
...
2. 点击 `选择路径底图与层级`，选择目标所在的基础底图或 tier。路线已有点后选择器会锁定，避免在不转换已有坐标的情况下静默更换坐标系。
3. 选择 `加 NAVMESH/选择`，在地图上点击一个或多个目标。基础底图上的点按 base 坐标导出；真实 tier 上的点会同时导出 `target_tier`。
4. 点击 `规划当前片段`，查看 C++ 运行时展开的步行路线；需要评估滑索时，先勾选 `启用滑索规划（桌面端）`。
5. 点击 `复制路径`；启用滑索后按钮会改为 `复制完整参数`，并一并导出 `zip: true`。

`Delete` 删除选中的作者路点，`清除规划` 只移除预览，不会修改作者路径。
...
`路线制作` 工作区中的“实机试跑”面板对两个制作页签可用，跑的永远是**当前页签里那一份**：路径编辑把当前作者路径原样交给 `MapNavigateAction`，断言模式把画出的框原样交给 `MapLocateAssertLocation` 认一次，人在框里就提示通过、不在就提示不通过，不加额外判定。改完自动重新装载，不必手动同步。切到 `日志分析` 工作区后，连接与试跑面板会隐藏且连接状态探测暂停。
