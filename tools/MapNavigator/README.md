...
三个路线制作页签都可通过 `选择项目节点` 扫描当前项目 `assets`：路径编辑载入 `MapNavigateAction` 继续编辑，A* 在其中同一底图至少有两个坐标时将它们作为关键点并立即规划（单坐标保留为目标预览点），断言模式默认选择 `MapLocateAssertLocation`，也可切换到 `参考路线` 查看导航路径。列表支持按资源路径、节点名或区域搜索；各页签的 `更多` 可读取当前剪贴板中的 JSON，并自动识别完整 Pipeline、单个导航或断言节点、`{"path": [...]}` 与纯 `path` 数组。
