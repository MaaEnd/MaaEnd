...
- 路径编辑页显示当前底图的 navmesh 网格，可标出游戏内当前位置作为只读参考点，并按运行时语义规划当前区域片段；规划可选择是否启用桌面端滑索，预览不会改写作者路点。
...
路径编辑页始终显示当前底图与层级对应的 navmesh 网格。点击 `规划当前片段`，会以当前区域片段的第一个作者路点为离线预览起点，将该片段的完整 `path` 交给 C++ `MapNavigateAction` 展开器。粉色折线表示运行时生成的步行段，青色有向边表示选中的滑索跳；`required`、`target_tier`、`target_deck_y` 与动作边界均使用实际运行时语义。单独选中 `NAVMESH` 路点时，属性面板会查询该坐标的重叠可走面，可预览后选择或清除 `target_deck_y`；移动目标、切换坐标层级或改成其他动作会清除旧的目标面声明。跨区域路线按页面当前片段分别预览，避免把不同底图坐标画在一起。
