...
5. 单击一座滑索架选择 A 点，再单击另一座选择 B 点；地图会画出测距虚线，侧栏按运行时同款占地算法列出可能中心跨度范围、原始锚点距离、水平距离范围、高差、底图像素距离和几何连接判定。拖动地图仍是平移，再次单击 A 或点击「清除」可取消选择。
...
- `索1、索2……` 圆点：运行日志中可定位的本次选择滑索架。青色实心表示实际已经过，琥珀空心表示日志未确认经过；侧栏同时列出 `[x, y, height]`，高度会优先从同组 ZIP 快照的同位置标记补齐。
- 紫色菱形：同组 ZIP 的 `record/Ziplines.json` 经仓库标定投影后的候选滑索架。深色描边和白色中心点用于在浅色底图上保持清晰；这里只按地图与滑索模板类型筛选，可能仍包含未供电、楼层不符、不可达或被成本规则排除的架子。
- A / B 外圈与两点间虚线：手动选择的测距对象。森空岛记录的是随朝向变化的角格锚点，因此主结果按两端 `footprint` 显示可能中心的世界三维跨度范围（米）；高度差原样参与，不应用占地偏移。与运行时一致，同类型、同 `level_id` 且范围下界不超过该类型 `max_span` 时只标为“几何上可连接”。这不代表供电、可达性、禁用边与成本规划均已通过，实际能否采用仍以运行时决策为准。
- 青色虚线：接近上索点与离开末端滑索的端点连接估计。当前日志没有记录这两段的完整折线，因此它们不是角色逐帧轨迹。

运行日志不会为分析功能额外输出完整滑索快照：本次选择只使用既有的首尾架、实际发射落点和落地确认，未执行链的中间架可能只能通过候选背景点与架数判断。ZIP 只解压 `cpp-algo/debug/maafw*.log` 和 `record/Ziplines.json`，逐条解析后释放日志文本；其他附件会被跳过。所有日志和滑索快照均由浏览器本地读取，不会上传到后端。日志模式只允许缩放和平移，也不会向实机试跑装载路线。

## 实机试跑

`路线制作` 工作区中的“实机试跑”面板对三个制作页签可用，跑的永远是**当前页签里那一份**：路径编辑跑编辑器路点，A\* 跑「复制配置」会给出的那串 `NAVMESH` 目标（同一处口径，两者不会漂移），断言模式跑的是画出来的那个框——原样交给 `MapLocateAssertLocation` 认一次，人在框里就提示通过、不在就提示不通过，不加额外判定。改完自动重新装载，不必手动同步。切到 `日志分析` 工作区后，连接与试跑面板会隐藏且连接状态探测暂停。
