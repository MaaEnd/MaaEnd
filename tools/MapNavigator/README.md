...
左侧「连接状态」下方、页签之上的“实机试跑”面板对三个可编辑页签可用，跑的永远是**当前页签里那一份**：路径编辑跑编辑器路点，A\* 跑「复制配置」会给出的那串 `NAVMESH` 目标（同一处口径，两者不会漂移），断言模式跑的是画出来的那个框——原样交给 `MapLocateAssertLocation` 认一次，人在框里就提示通过、不在就提示不通过，不加额外判定。改完自动重新装载，不必手动同步。日志分析是只读视图，不参与试跑。
