...
目标周围还有别的角色、只有图标模板匹配时容易点到别人时，在对应终点条目上设置 `"verify_name": true`。世界交互提示会把目标名称渲染在交货图标旁的提示区域里，开启后到达判定除图标模板外还要求提示文本命中该终点的名称；名称按运行时目录里的五语言文本匹配，因此不受游戏语言影响。只有 NPC 终点的提示会显示角色名，其他 `kind` 开启时生成器直接报错。
