...
- `assets/resource/pipeline/AutoDelivery/RecycleBins.json`：按区域生成四号谷地资源回收站任务的二次判定流程；`AutoDeliveryRecognizeDestination` 发现同区两个回收站后开始追踪或打开任务地图，使用数据源中的 `u/v` 坐标与 `MapFind` 检查 `RecycleBin` 图标，并把命中的精确终点交给既有路线分发节点；
