...
MaaEnd 使用三份由 zmdmap 数据 CI 生成并发布的精简数据：
...
`fetch-data.mjs` 根据 zmdmap 版本接口和 `data/version.txt` 下载这三份文件。版本未变化且本地缓存完整时跳过下载；网络检查失败时保留当前缓存。
...
完整生成某个任务时使用对应的 pnpm 命令。三个命令都会先同步 zmdmap 数据，再执行当前任务自己的生成链路：
...
`data/scripts/` 中的三个 Python 入口属于 zmdmap 数据生产侧，不是 MaaEnd 日常生成流程的一部分。它们彼此独立，只读取当前任务需要的 TableCfg，并输出不含版本或来源元数据的纯业务 JSON：
...
未指定 `--table-cfg-dir` 时默认读取当前用户的 `Downloads/TableCfg`。生成结果与已有文件相同时跳过写入；`--force` 可强制重写。`data/scripts/tablecfg_utils.py` 只负责本地文件读取、五语言文本引用、结果比较和 JSON 写入，任务业务规则分别留在三个入口中。
