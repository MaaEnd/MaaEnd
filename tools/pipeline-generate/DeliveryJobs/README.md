...
| `PackCargoOnly` | `buildModeOverride` | 只启用货物入口；`cargoExpected=PACK_CARGO_EXPECTED`（不含「查看报价」）；`bidAction=DeliveryJobsBackToDepotFromBid`；`ongoingDeliveryAction=DeliveryJobsSkipOngoingDelivery` |
