...
按报价处理默认以整数 `119000`（即 11.9 万）为阈值。“达到或高于阈值时”默认接取并转交，“低于阈值时”默认仅接取委托；两侧均可改为“接取并转交”“仅接取委托”或“不处理”。武陵城和试验园区的各报价分支还可独立选择“全自动送货”。“仅接取委托”会在接取后返回仓储节点继续遍历，“不处理”会关闭报价页并继续遍历。报价 OCR 失败时会停在报价页并提示用户，不会自动接取。

全自动送货使用 `AutoDelivery` 通用组件完成取货、目的地识别、MapNavigator 寻路和提交货物，当前支持武陵城和试验园区。DeliveryJobs 通过 `AutoDeliveryOpenCurrentJobDetail` 注入对应本地仓储节点的“查看任务”入口，不经过运送委托列表。风险确认与滑索偏好仅对 `Win32-Front` / `Wlroots` 控制器开放；受项目接口能力限制，其他控制器仍可能显示“全自动送货”处理方式，但无法关闭默认安全守卫，选择后会直接停止任务。滑索偏好只允许 MapNavigator 在预计更快且满足供电、上下索条件时规划滑索，不保证每条路线都会使用。送货成功后返回 DeliveryJobs 仓储节点循环，送货失败则停止整个任务，避免继续装箱。该功能仍处于测试阶段，使用前必须确认风险提示。
