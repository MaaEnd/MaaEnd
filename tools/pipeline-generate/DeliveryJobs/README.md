...
全自动送货只使用 AutoDelivery 唯一公共入口 `AutoDelivery`，支持四号谷地和武陵的全部仓储节点。DeliveryJobs 负责打开对应本地仓储节点的“查看任务”入口，并为提交完成配置 anchor；识别、传送、寻路、取货与提交均由 AutoDelivery 完成，不把完整链路装入一个 `SubTask`。
