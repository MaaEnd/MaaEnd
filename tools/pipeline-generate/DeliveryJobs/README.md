...
全自动送货复用 `SeizeDeliveryJobs` 的取货、目的地识别、MapTracker 寻路和提交货物流程，当前支持武陵城和试验园区。风险确认与滑索设置仅对 `Win32-Front` / `Wlroots` 控制器开放；受项目接口能力限制，其他控制器仍可能显示“全自动送货”处理方式，但无法关闭默认安全守卫，选择后会直接停止任务。启用后会返回对应本地仓储节点并点击“查看任务”，不经过运送委托列表；送货成功后返回 DeliveryJobs 仓储节点循环，送货失败则停止整个任务，避免继续装箱。该功能仍处于测试阶段，使用前必须确认风险提示。
