...
按报价处理默认以 `11.9万` 为阈值。“达到或高于阈值时”默认接取并转交，“低于阈值时”默认仅接取委托；两侧均可改为“接取并转交”“仅接取委托”或“不处理”。“仅接取委托”会在接取后返回仓储节点继续遍历，“不处理”会关闭报价页并继续遍历。报价 OCR 失败时会停在报价页并提示用户，不会自动接取。以后接入自动送货时，可在共用的报价动作选项中增加对应动作，让两个报价分支按需选择。
