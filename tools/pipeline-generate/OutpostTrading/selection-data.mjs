...
const OUTPUT_PATH = resolve(__dirname, "../../../assets/data/OutpostTrading/selection_data.json");
...
export function buildSelectionItems(data = settlementData, sourceLocations = outpostTradingLocations) {
    const items = {};
    const locations = {};
...
export function buildOutpostTradingSelectionData() {
    const operators = {};
    for (const [
        ,
        operator,
    ...
    return {
        ...
        operators,
        location_order: outpostTradingLocationsNewestFirst.map((location) => location.LocationId),
        locations,
    };
}

export const outpostTradingSelectionData = buildOutpostTradingSelectionData();
...
function buildSelectableItems() {
    const items = [];
    const seen = new Set();
    for (const location of outpostTradingLocationsNewestFirst) {
        const settlement = settlementData.settlements[location.SettlementId];
        for (const level of settlement.prosperity_levels || []) {
            for (const tradeItem of level.trade_items || []) {
                ...
                if (
                    ...
                    !outpostTradingSelectionData.items[itemID]
                ) {
                    continue;
                }
                seen.add(itemID);
...
export const outpostTradingItemLocaleEntries = outpostTradingSelectableItems.map(({id}) => ({
    ...
    names: outpostTradingSelectionData.items[id]?.names || {},
}));

export function writeOutpostTradingSelectionData() {
    mkdirSync(dirname(OUTPUT_PATH), {recursive: true});
    writeFileSync(OUTPUT_PATH, `${JSON.stringify(outpostTradingSelectionData, null, 4)}\n`, "utf8");
    console.log(`[OutpostTrading] 已生成运行时选品数据：${OUTPUT_PATH}`);
}

if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
    writeOutpostTradingSelectionData();
}
