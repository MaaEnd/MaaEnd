...
import {outpostTradingLocationsNewestFirst} from "./model.mjs";
import {
...
const generatedPath = new URL("../../../assets/data/OutpostTrading/selection_data.json", import.meta.url);

test("OutpostTrading selection data artifact matches the current source model", () => {
    const generated = JSON.parse(readFileSync(generatedPath, "utf8"));
    assert.deepEqual(generated, buildOutpostTradingSelectionData());
    assert.deepEqual(generated, outpostTradingSelectionData);
});

test("OutpostTrading selection data contains only valid stable references", () => {
    const data = outpostTradingSelectionData;
    assert.deepEqual(
        data.location_order,
        outpostTradingLocationsNewestFirst.map((location) => location.LocationId),
    );
...
test("OutpostTrading temporary activity items stay recognizable but are not selectable or sellable", () => {
    const excluded = [
        "item_activity_xiranite_enr_hulu",
        "item_activity_xiranite_hulu",
    ];
    const selectableIDs = new Set(outpostTradingSelectableItems.map((item) => item.id));
    for (const itemID of excluded) {
        assert.ok(outpostTradingSelectionData.items[itemID]);
...
test("OutpostTrading generated operator order follows feature matches then descending character id", () => {
    const catalog = {
...
test("OutpostTrading generated operator order rejects missing or malformed character ids", () => {
...
test("OutpostTrading generated Refugee Camp restore order matches the observed in-game list", () => {
    assert.deepEqual(outpostTradingSelectionData.locations.RefugeeCamp.restore_operators, [
        "Laevatain",
        "Camille",
        "Antal",
        "Rossi",
...
test("OutpostTrading generated target operators preserve equal bonus tiers", () => {
    const data = outpostTradingSelectionData.locations.XiranflowCloudseederStation.target_operators;
