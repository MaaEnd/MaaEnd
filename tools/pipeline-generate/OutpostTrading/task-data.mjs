...
const ITEMS = {};
for (const item of outpostTradingSelectableItems) {
...
const SETTLEMENT_REGION_MAP = outpostTradingLocations.reduce((acc, location) => {
...
const PRIORITY_ITEM_PRICE_BY_REGION = outpostTradingLocations.reduce((acc, location) => {
    acc[location.RegionPrefix] = acc[location.RegionPrefix] || new Map();
    for (const item of outpostTradingSelectionData.locations[location.LocationId]?.items || []) {
...
const LOCATIONS = outpostTradingLocations;
...
function buildReserveModeCases(slot) {
    return [
        {
            name: "Quantity",
            label: "$task.OutpostTrading.ReserveModeQuantity",
            option: [`SellProductReserveItem${slot}Value`],
        },
        {
            name: "NeverSell",
            label: "$task.OutpostTrading.ReserveModeNeverSell",
            pipeline_override: {
                [`OutpostTradingRegisterReserveRule${slot}`]: {
                    custom_action_param: {
                        operation: "register",
...
function buildPriorityItemCases(regionPrefix, slot) {
    ...
    return [
        {
            name: "None",
            label: "$task.OutpostTrading.PriorityNone",
        },
        ...Object.values(ITEMS)
...
function buildSelectionStrategyCases() {
    return [
        {
            name: "Rarity",
            label: "$task.OutpostTrading.SelectionStrategyRarity",
            pipeline_override: {
        ...
        {
            name: "Price",
            label: "$task.OutpostTrading.SelectionStrategyPrice",
            pipeline_override: {
                OutpostTradingConfigureSelectionStrategy: {
                    custom_action_param: {
                        operation: "configure_strategy",
        ...
        {
            name: "Stock",
            label: "$task.OutpostTrading.SelectionStrategyStock",
    ...
    return [
        {
            ...
            pipeline_override: {
                [`OutpostTrading${regionPrefix}InitializePrioritySession`]: {
                    custom_action_param: {
...
export const outpostTradingTaskRows = LOCATIONS.map((loc, index) => {
...
export default outpostTradingTaskRows;
