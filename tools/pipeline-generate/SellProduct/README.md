...
`task-template.jsonc` 只保留全局物品优先级、独立物品保留规则、强制刷新干员缓存，以及地区/据点售卖开关。全局优先级默认关闭，此时每个启用据点按默认货品执行前两次售卖；开启后，具体货品只启用实际出售该货品的据点，`Auto` 则按每个据点自己的稀有度和单价顺序生成对应货品。

物品保留规则提供 4 个独立槽位，每个槽位选择具体货品和最低保留数量。保留规则按实际识别到的 `itemId` 生效，与优先级顺位无关，默认选品也会遵守；数量 `0` 表示全部可售，同一物品重复配置时后面的槽位覆盖前面的槽位。
