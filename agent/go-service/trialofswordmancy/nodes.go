...
const (
	...
	// 决策 → 执行映射的 Do 入口节点（新增）。
	nodeDoDrawCard     = "TrialOfSwordmancyDoDrawCard"
	nodeDoDrawCardConf = "TrialOfSwordmancyDoDrawCardConfirm"
	nodeDoDoubleReward = "TrialOfSwordmancyDoDoubleReward"

	// 复用的既有执行链节点。
	nodeGiveUp    = "TrialOfSwordmancyDailyGiveUp" // 放弃 → 确认 → 重置寻路 → 回主入口
	nodeStartTrial = "TrialOfSwordmancyStartTrial" // 开始演算 → 编队 → 战斗 → 领奖
	nodeFinish    = "TrialOfSwordmancyDailyFinish" // 演算结束（奖励耗尽）
)
...
// TODO(需真机截图校准): 以下为「剩余次数 / 翻倍态」识别所需的 ROI 与模板，与 Point1-5.png
// 同属识别前置数据。未校准前 recognition 读不到对应值，State 相关字段为 0/false，
// Decide 会判定状态不可达并安全结束任务（见 action.go）。校准时只需改这里的常量。
var (
