...
func (a *CreditShoppingParseParams) Run(ctx *maa.Context, arg *maa.CustomActionArg) bool {
	...
	getNodeAttach := func(nodeName string) map[string]interface{} {
		...
		attachRaw, ok := nodeData["attach"].(map[string]interface{})Collapse commentComment on line R84MistEO commented on Feb 5, 2026 MistEOon Feb 5, 2026ContributorMore actions@dongwlin 这个是不是有结构化的数据？需要手动解析吗Reactdongwlin replied on Feb 5, 2026 dongwlinon Feb 5, 2026ContributorMore actions有一个GetNodeData，可以不用手动解析node的json，但attach是不确定的（？）这一部分得手动解析ReactWrite a replyResolve comment
		if !ok {
