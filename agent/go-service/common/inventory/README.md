...
`InventoryMoveRepoItemToBagAction` 是仓库到背包的物品移动动作：调用方在同一帧识别仓库源格和背包目标格，桌面端直接拖动，ADB 端先长按源格并等待操作菜单确认长按生效，再保持同一触点拖到目标格。它不属于 Ctrl/Shift/Alt 三种数量转移，也不负责物品识别、翻页或数量判断。

前置条件是调用方已进入支持物品转移的界面，并在同一帧识别出 `StashBackpackFindCurrentItemInRepo` 与 `StashBackpackFindCurrentItemInBag`。动作不负责导航、仓库切换、物品扫描、分页或快照维护。
...
动作根据控制器类型分支。仓库到背包动作在 ADB 上按以下顺序完成：

1. 在菜单弹出前已经由 `And` 识别并缓存仓库源格和背包目标格。
2. 触点 0 持续按住仓库源物品，截图识别操作菜单；先查左侧，未命中才查同一帧的右侧。
3. 菜单出现后保持触点 0，直接移动到缓存的背包目标格。
4. 到达目标格后保持约 200 毫秒，再释放触点 0。
...
数量转移动作（Ctrl / Shift / Alt）仍使用第二触点点击操作菜单，不与仓库到背包动作混用。
...
桌面端使用滚轮，ADB 覆盖为沿列间隙的触摸滑动；纵向移动后保持触点并横移 50 像素，再停留 200 毫秒抬起，以消除惯性。调用方负责等待网格稳定、滚动条边界识别、页计数与停止条件，避免不同阶段各自复制手势坐标。快照、搜索和验证必须使用同一组手势，保持页面划分一致。
