...
func drawRectangle(dst *image.RGBA, rect image.Rectangle, c color.RGBA) {
	drawLine(dst, rect.Min, image.Pt(rect.Max.X-1, rect.Min.Y), c)
	drawLine(dst, image.Pt(rect.Max.X-1, rect.Min.Y), rect.Max.Sub(image.Pt(1, 1)), c)
	drawLine(dst, rect.Max.Sub(image.Pt(1, 1)), image.Pt(rect.Min.X, rect.Max.Y-1), c)
	drawLine(dst, image.Pt(rect.Min.X, rect.Max.Y-1), rect.Min, c)
}

func drawLine(dst *image.RGBA, from, to image.Point, c color.RGBA) {
