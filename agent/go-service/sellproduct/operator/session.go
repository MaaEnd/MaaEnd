...
func (a *OperatorSessionAction) Run(ctx *maa.Context, arg *maa.CustomActionArg) bool {
	...
	case operatorSessionOperationEnterLocation:
		...
		operatorSessionEnterLocation(p.Location)
	case operatorSessionOperationCompleteTarget:
		...
		printRuntimeOperatorAssignment(ctx, p.Location, operatorUsageTarget, candidate, p.Changed)
	case operatorSessionOperationCompleteRestore:
		...
		printRuntimeOperatorAssignment(ctx, p.Location, operatorUsageRestore, candidate, p.Changed)
...
func parseOperatorSessionActionParam(arg *maa.CustomActionArg) (*operatorSessionActionParam, error) {
	...
	if p.Operation == operatorSessionOperationExcludeSelected &&
		p.Usage != operatorUsageTarget && p.Usage != operatorUsageRestore {
...
func operatorSessionClaimCacheRescan() bool {
	return operatorSessionClaimRetry(operatorUsageAll, "global")
}
