...
// Pipeline 决定据点的执行顺序，recognition.go 负责把识别结果交给流程，
...
// operatorSelectionParam 是补齐资源数据和任务状态后的运行时选择参数。
...
func resolveOperatorSelectionParam(p *operatorRecognitionParam) (*operatorSelectionParam, error) {
	...
	case operatorUsageRestore, operatorUsageAll:
	default:
...
func recordTargetAssignment(p *operatorRecognitionParam, candidate operatorCandidate) {
	if p.Usage == operatorUsageTarget {
