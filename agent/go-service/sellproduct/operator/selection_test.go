...
func TestCachedOutpostProsperityChangesGeneratedSellingOperator(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			selection, err := resolveOperatorSelectionParam(&operatorRecognitionParam{
				Usage:    operatorUsageTarget,
				Location: location,
			})
			if err != nil {
...
func TestTargetSelectionMinimizesGlobalOperatorChangesWithinBestBonusTier(t *testing.T) {
	...
	selection := *p
	selection.Usage = operatorUsageRestore
...
func TestPlanningMatches20260719LogSnapshot(t *testing.T) {
	...
	for _, location := range locationOrder {
		targetSelection, resolveErr := resolveOperatorSelectionParam(&operatorRecognitionParam{
			Usage:    operatorUsageTarget,
			Location: location,
		})
		if resolveErr != nil {
		...
		restoreSelection, resolveErr := resolveOperatorSelectionParam(&operatorRecognitionParam{
			Usage:    operatorUsageRestore,
			Location: location,
		})
		if resolveErr != nil {
...
func TestPlanningMatches20260720ProsperityMaxLogSnapshot(t *testing.T) {
	...
	for _, location := range locationOrder {
		...
		current, _, ok := findCurrentBestOperator(
			equivalent,
			targetSelection.KnownOperators,
			[]ocrmatch.Item{{Text: currentOCR[location]}},
		)
...
func TestReconstructionReplanProtectsInfraRestoreFrom20260719NoonLog(t *testing.T) {
	...
	if excluded, ok := operatorSessionExcludeSelected(operatorUsageTarget, "ReconstructionHQ"); !ok || excluded.Name != "Pogranichnik" {
