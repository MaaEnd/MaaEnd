...
func (r *OperatorConflictRecognition) Run(
	...
	items := ocrmatch.CollectResults(prompt)
...
func parseOperatorConflictParam(raw string) (*operatorConflictParam, error) {
	...
	if p.Usage != operatorUsageTarget && p.Usage != operatorUsageRestore {
...
func findOperatorConflictSource(
	...
	for _, item := range ocrmatch.SortItemsByPosition(items) {
		text := ocrmatch.StripSeparators(item.Text)
		if text == "" {
			continue
		}
		for _, locationName := range data.LocationOrder {
			...
			for _, expected := range selectiondata.ExpectedNames(entry.Names) {
				candidate := ocrmatch.StripSeparators(expected)
