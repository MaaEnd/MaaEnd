...
func TestGoodsSelectionExhaustionKeepsStableObservation(t *testing.T) {
	resetPrioritySelectionSession()
	beginGoodsSelection("Outpost")
	setGoodsSelectionExhaustedItems("Outpost", []string{"item"})

	if _, ok := buildGoodsSelectionExhaustedResult("Outpost"); ok {
		t.Fatal("first exhaustion observation must remain unconfirmed")
	}
	if _, ok := buildGoodsSelectionExhaustedResult("Outpost"); !ok {
		t.Fatal("second identical exhaustion observation must be confirmed")
	}
}
