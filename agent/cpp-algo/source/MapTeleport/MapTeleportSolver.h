...
public:
    ...
    std::optional<AnchorHit>
        ConfirmAnchor(const cv::Mat& screen, const cv::Point2d& expected, double viewportScale, const AnchorConfig& cfg);

    std::optional<CoreIconHit>
        ConfirmCoreIcon(const cv::Mat& screen, const cv::Point2d& expected, double viewportScale, const CoreIconConfig& cfg);
...
private:
    ...
    const cv::Mat* LoadAnchorTemplate();
    const std::vector<CoreTemplate>* LoadCoreTemplates();
    ...
    bool _anchorTemplateTried = false;
    std::vector<CoreTemplate> _coreTemplates;
    bool _coreTemplatesTried = false;
};
