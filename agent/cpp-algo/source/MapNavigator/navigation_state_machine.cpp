...
void LogNavigationSnapshot(const NavigationSnapshot& snapshot, const char* reason)
{
    ...
    LogInfo << "Snapshot."
            ...
            << VAR(error_yaw)
            << VAR(stalled_ms)
            << VAR(recovery_cooldown_ms)
            << VAR(advanced_by_pass_through)
            << VAR(portal_commit_ready)
            << VAR(is_zone_transition_isolated)
...
bool NavigationStateMachine::Run()
{
    ...
    while (!should_stop_()
           && session_->phase() != NaviPhase::Finished
           && session_->phase() != NaviPhase::Failed) {
    ...
    if (!should_stop_()
        && param_.is_exact_target
        && !session_->current_path().empty()
        && session_->phase() != NaviPhase::Failed) {
...
bool NavigationStateMachine::Bootstrap()
{
    if (param_.enable_rejoin || param_.path_trim) {
        const RouteRejoinPlan initial_rejoin_plan =
            rejoin_planner_->Plan(*position_, position_->angle, session_->original_path(), 0);
        if (initial_rejoin_plan.abort || initial_rejoin_plan.candidates.empty()) {
            ...
            LogError << "Initial route rejoin failed."
                     << VAR(rejoin_abort)
                     << VAR(nearest_route_distance);
            return false;
        }
...
bool NavigationStateMachine::TickWaitRelocation()
{
    ...
    const auto waited_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - relocation_wait_started_).count();
    ...
    if (!position_provider_->Capture(position_, false, session_->CurrentExpectedZone())) {
        if (waited_ms > kRelocationWaitTimeoutMs) {
            ...
            LogError << "Relocation wait timed out before position stabilized."
                     << VAR(current_node_idx)
                     << VAR(waited_ms);
            return true;
        }
    ...
    if (held_fix) {
        ...
        if (waited_ms > kRelocationWaitTimeoutMs) {
            ...
            LogError << "Relocation wait timed out while locator fix stayed held."
                     << VAR(current_node_idx)
                     << VAR(waited_ms)
                     << VAR(held_fix_streak);
            return true;
    ...
    const double moved_from_anchor =
        std::hypot(position_->x - relocation_anchor_pos_.x, position_->y - relocation_anchor_pos_.y);
    const bool relocation_complete =
        !relocation_requires_movement_
        || position_->zone_id != relocation_anchor_pos_.zone_id
    ...
    if (!relocation_complete) {
        ...
        if (waited_ms > kRelocationWaitTimeoutMs) {
            session_->UpdatePhase(NaviPhase::Failed, "relocation_wait_timeout");
            const size_t current_node_idx = session_->current_node_idx();
            LogError << "Relocation wait timed out without movement."
                     << VAR(current_node_idx)
                     << VAR(waited_ms)
                     << VAR(moved_from_anchor);
            return true;
        }
        SleepFor(kRelocationRetryIntervalMs);
        return true;
    ...
    if (relocation_completion_policy == RelocationCompletionPolicy::RejoinOrFail) {
        ...
        LogInfo << "Relocation recovery completed, attempting route rejoin."
                << VAR(relocation_reason)
                << VAR(current_node_idx)
                << VAR(position_x)
                << VAR(position_y);
        ...
        LogError << "Relocation recovery could not rejoin route; aborting blind resume."
                 << VAR(relocation_reason)
                 << VAR(current_node_idx)
                 << VAR(position_x)
                 << VAR(position_y);
...
bool NavigationStateMachine::TickAdvanceOnRoute()
{
    ...
    if (!motion_controller_->IsMoving()) {
        ...
        const bool stationary_portal_commit_ready =
            ...
            && std::hypot(current_waypoint.x - position_->x, current_waypoint.y - position_->y)
                <= kPortalCommitDistance;
        if (CanArriveSamePointAction(current_waypoint)) {
            const double stationary_distance =
                std::hypot(current_waypoint.x - position_->x, current_waypoint.y - position_->y);
    ...
    const bool allow_blind_recovery =
        current_waypoint.action == ActionType::PORTAL || session_->is_waiting_for_zone_switch();
    const auto enter_black_screen_recovery = [&](const char* capture_stage) {
        if (!motion_controller_->IsMoving() || allow_blind_recovery
        ...
        const std::string zone_id =
            position_->valid ? position_->zone_id : session_->current_zone_id();
        LogWarn << "Black screen detected while moving, delaying motion recovery."
                << VAR(current_node_idx)
                << VAR(capture_stage)
                << VAR(held_fix)
                << VAR(zone_id);
        EnterRelocationWait(
            ...
            kRespawnRecoveryPauseMs,
            false,
            RelocationCompletionPolicy::RejoinOrFail);
        return true;
    };
    ...
    if (
        current_waypoint.RequiresStrictArrival()
        && strict_arrival_walk_reset_node_idx_ != session_->current_node_idx()
        && actual_distance <= kStrictArrivalWalkResetDistance
    ...
    const double yaw_mismatch =
        std::abs(NaviMath::NormalizeAngle(visual_yaw - session_->virtual_yaw()));
    const bool advanced_by_pass_through =
        session_->ShouldAdvanceByPassThrough(session_->current_node_idx(), position_->x, position_->y);
    const double portal_distance =
        session_->DistanceToAdjacentPortal(session_->current_node_idx(), position_->x, position_->y);
    const bool is_zone_transition_isolated = portal_distance <= kZoneTransitionIsolationDistance;
    const bool portal_commit_ready =
        current_waypoint.action == ActionType::PORTAL
        && actual_distance <= kPortalCommitDistance;
    ...
    if (
        !is_zone_transition_isolated && param_.arrival_timeout > 0 && actual_distance > kNoProgressMinDistance
        && stalled_ms > param_.arrival_timeout
        && session_->no_progress_recovery_attempts() >= kArrivalTimeoutMinRecoveryAttempts) {
        ...
        LogError << "Arrival watchdog exhausted after repeated recovery attempts."
                 << VAR(current_node_idx)
                 << VAR(actual_distance)
                 << VAR(best_actual_distance)
                 << VAR(stalled_ms)
                 << VAR(no_progress_recovery_attempts);
        LogNavigationSnapshot(snapshot, "arrival_watchdog_exhausted");
        return true;
    }

    if (
        distance < current_waypoint.GetLookahead()
    ...
    const double moved_distance =
        session_->has_previous_driver_pos() && session_->previous_driver_pos().zone_id == position_->zone_id
            ? std::hypot(
                  position_->x - session_->previous_driver_pos().x,
                  position_->y - session_->previous_driver_pos().y)
            : 0.0;
    const double distance_delta =
        std::isnan(session_->previous_driver_distance())
            ? 0.0
            : session_->previous_driver_distance() - actual_distance;
    const double distance_increase =
        std::isnan(session_->previous_driver_distance())
            ? 0.0
            : actual_distance - session_->previous_driver_distance();
    const bool suspected_respawn =
        session_->has_previous_driver_pos()
        ...
        && current_waypoint.action != ActionType::PORTAL
        && !current_waypoint.WaitsForRelocation()
    ...
    if (partial_respawn_signal) {
        const size_t current_node_idx = session_->current_node_idx();
        LogWarn << "Respawn-like relocation signal observed, but thresholds not fully met."
                << VAR(current_node_idx)
                << VAR(moved_distance)
                << VAR(distance_increase)
                << VAR(actual_distance)
                << VAR(held_fix);
    }
    if (suspected_respawn) {
        const size_t current_node_idx = session_->current_node_idx();
        LogWarn << "Respawn-like relocation detected, delaying motion recovery."
                << VAR(current_node_idx)
                << VAR(moved_distance)
                << VAR(distance_increase)
                << VAR(actual_distance);
    ...
    const double error_yaw =
        NaviMath::NormalizeAngle(ideal_yaw_now - session_->virtual_yaw());
    ...
    const bool post_turn_forward_commit_active =
        motion_controller_->IsForwardCommitActive(loop_now);
    ...
    if (
        !is_zone_transition_isolated && driver_decision.request_rejoin
        && recovery_cooldown_ms > param_.rejoin_retry_timeout
        ...
        const double best_actual_distance = session_->best_actual_distance();
        const int no_progress_recovery_attempts = session_->no_progress_recovery_attempts();
        LogWarn << "LocalDriverLite escalated to route rejoin."
                << VAR(current_node_idx)
                << VAR(actual_distance)
                << VAR(best_actual_distance)
                << VAR(stalled_ms)
        ...
        LogError << "Route rejoin failed after local recover exhaustion."
                 << VAR(failed_current_node_idx)
                 << VAR(actual_distance)
                 << VAR(stalled_ms);
    ...
    const bool allow_turn_in_place =
        !held_fix && !post_turn_forward_commit_active && !is_zone_transition_isolated
    ...
    if (allow_turn_in_place) {
        ...
        const double best_actual_distance = session_->best_actual_distance();
        LogWarn << "Turn-in-place correction triggered."
                << VAR(current_node_idx)
                << VAR(position_x) << VAR(position_y)
                << VAR(target_x) << VAR(target_y)
                << VAR(next_position_idx)
                << VAR(sensor_target_yaw)
                << VAR(visual_yaw)
                << VAR(virtual_yaw)
                << VAR(advanced_by_pass_through)
                << VAR(sensor_yaw_error) << VAR(actual_distance)
                << VAR(best_actual_distance)
        ...
        if (next_position_idx < session_->current_path().size()) {
            ...
            LogWarn << "Turn-in-place next waypoint."
                    << VAR(next_waypoint_x) << VAR(next_waypoint_y)
                    << VAR(next_waypoint_zone_id);
        }
        motion_controller_->Stop();
        local_driver_->CancelCommitment();
        ...
        const double turn_in_place_delta =
            NaviMath::NormalizeAngle(sensor_target_yaw - session_->virtual_yaw());
        motion_controller_->InjectMouseAndTrack(
            turn_in_place_delta,
            severe_turn_drift,
    ...
    const bool suppress_micro_turn =
        held_fix
        || position_provider_->HeldFixStreak() > 0
        || post_turn_forward_commit_active
    ...
    if (held_fix && position_provider_->HeldFixStreak() == 1) {
        ...
        LogWarn << "Held locator fix detected, suppress steering corrections."
                << VAR(current_node_idx)
                << VAR(zone_id)
    ...
    if (session_->straight_stable_frames() > kStableFramesThreshold) {
        if (
            latency_ms < kMaxLatencyForCorrectionMs
            && std::abs(NaviMath::NormalizeAngle(visual_yaw - session_->virtual_yaw()))
                < kMaxYawDeviationForCorrection) {
    ...
    if (param_.enable_local_driver) {
        const LocalDriverAction target_drive_action =
            post_turn_forward_commit_active
                ? LocalDriverAction::Forward
                : driver_decision.action;
        const bool force_drive_action =
            post_turn_forward_commit_active
    ...
    const bool auto_sprint_ready =
        param_.sprint_threshold > 0.0
        && actual_distance > param_.sprint_threshold
        && std::abs(sensor_yaw_error) <= kLocalDriverSideAvoidYawDegrees
        && !held_fix
        && !is_zone_transition_isolated
        && !post_turn_forward_commit_active
        && !current_waypoint.RequiresStrictArrival()
        && motion_controller_->IsMoving()
        && (!param_.enable_local_driver
            || (driver_decision.state == LocalDriverState::Cruise
                && driver_decision.action == LocalDriverAction::Forward
                && !driver_decision.commitment_active));
    ...
    const std::chrono::steady_clock::time_point& now)
{
    if (last_auto_sprint_time_.time_since_epoch().count() > 0
        && std::chrono::duration_cast<std::chrono::milliseconds>(now - last_auto_sprint_time_)
                .count()
...
bool NavigationStateMachine::TickExactTargetRefine()
{
    const auto exact_target_it = std::find_if(
        session_->current_path().rbegin(),
        session_->current_path().rend(),
        [](const Waypoint& waypoint) { return waypoint.HasPosition(); });
    ...
    while (!should_stop_()) {
        ...
        const auto elapsed =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now() - exact_start_time)
                .count();
...
{
    if (sync_with_sensor_yaw && session_->HasCurrentWaypoint()
        && session_->CurrentWaypoint().IsHeadingOnly()) {
    ...
    while (session_->HasCurrentWaypoint() && session_->CurrentWaypoint().IsHeadingOnly()) {
        ...
        const double required_turn =
            NaviMath::NormalizeAngle(target_heading - session_->virtual_yaw());
        const size_t current_node_idx = session_->current_node_idx();
        const double virtual_yaw = session_->virtual_yaw();

        LogInfo << "HEADING node triggered."
                << VAR(current_node_idx)
                << VAR(target_heading)
                << VAR(virtual_yaw)
                << VAR(required_turn);
    ...
    bool portal_commit_ready)
{
    ...
    if (advanced_by_pass_through) {
        ...
        const double position_x = position_->x;
        const double position_y = position_->y;
        LogInfo << "Advance waypoint by pass-through."
                << VAR(current_node_idx)
                << VAR(actual_distance)
                << VAR(position_x)
                << VAR(position_y);
    ...
    if (portal_commit_ready) {
        const size_t current_node_idx = session_->current_node_idx();
        LogInfo << "Commit PORTAL approach early."
                << VAR(current_node_idx)
                << VAR(actual_distance)
                << VAR(portal_distance);
    }
    ...
    if (arrived_waypoint.WaitsForRelocation()) {
        ...
        if (session_->HasCurrentWaypoint()
            ...
            const size_t current_node_idx = session_->current_node_idx();
            LogInfo << "TRANSFER chained with same-point follow-up action."
                    << VAR(current_node_idx);
        ...
        EnterRelocationWait(
            ...
            0,
            true,
            RelocationCompletionPolicy::ResumeRoute);
        return true;
    }
    ...
    if (execution.entered_portal_mode) {
        if (
            session_->current_node_idx() + 1 < session_->current_path().size()
    ...
    relocation_finish_when_complete_ =
        should_enter_pending_transfer_wait && !session_->HasCurrentWaypoint();
    ...
    if (session_->phase() == NaviPhase::AdvanceOnRoute) {
        ...
        const bool can_chain_same_point_action =
            !motion_controller_->IsMoving()
            && CanChainSamePointAction(arrived_waypoint, next_waypoint);
        if (can_chain_same_point_action) {
            session_->ResetStraightStableFrames();
            return true;
        }
...
bool NavigationStateMachine::CanArriveSamePointAction(const Waypoint& waypoint) const
{
    ...
    return (waypoint.zone_id.empty() || waypoint.zone_id == position_->zone_id)
        && std::hypot(waypoint.x - position_->x, waypoint.y - position_->y)
            <= kSamePointActionChainDistance;
}

bool NavigationStateMachine::CanChainSamePointAction(
    const Waypoint& from_waypoint,
    const Waypoint& to_waypoint) const
{
    ...
    return (to_waypoint.zone_id.empty() || to_waypoint.zone_id == position_->zone_id)
        && std::hypot(to_waypoint.x - from_waypoint.x, to_waypoint.y - from_waypoint.y)
            <= kSamePointActionChainDistance;
}
...
bool NavigationStateMachine::ApplyRejoinCandidate(const RouteRejoinCandidate& candidate, const char* reason)
{
    ...
    LogInfo << "Route rejoin applied."
            << VAR(reason)
            << VAR(candidate_continue_index)
            << VAR(slice_start)
            << VAR(candidate_route_distance)
            << VAR(candidate_score);
...
bool NavigationStateMachine::AttemptRouteRejoin(const char* reason, bool require_forward_candidate)
{
    ...
    const RouteRejoinPlan plan =
        rejoin_planner_->Plan(*position_, position_->angle, session_->original_path(), preferred_index);
    if (plan.abort || plan.candidates.empty()) {
        ...
        LogWarn << "Route rejoin plan rejected."
                << VAR(reason) << VAR(plan_abort) << VAR(nearest_route_distance) << VAR(preferred_index);
        return false;
    }
    ...
    if (
        require_forward_candidate && selected->continue_index <= preferred_index
        && selected->decision == RejoinDecisionType::Waypoint) {
        const size_t selected_continue_index = selected->continue_index;
        LogWarn << "Route rejoin found no better forward candidate."
                << VAR(reason) << VAR(preferred_index) << VAR(selected_continue_index);
        return false;
    }
...
bool NavigationStateMachine::HasSevereTurnDrift(
    double yaw_mismatch,
    double actual_distance,
    int64_t stalled_ms) const
{
    return actual_distance > kAdaptiveActivationMinDistance
        ...
        && stalled_ms >= kAdaptiveActivationStallMs
        && yaw_mismatch >= kAdaptiveActivationSevereYawMismatchDegrees;
}

void NavigationStateMachine::ResumeMotionTowardsCurrentWaypoint(
    double origin_x,
    double origin_y,
    const char* reason,
    int settle_wait_ms)
