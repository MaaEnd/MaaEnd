...
void MotionController::ArmForwardCommit(double delta_degrees, const char* reason)
{
    if (session_->is_waiting_for_zone_switch()
        || std::abs(delta_degrees) < kPostTurnForwardCommitMinDegrees) {
        return;
    }

    turn_forward_commit_until_ =
        std::chrono::steady_clock::now()
        + std::chrono::milliseconds(kPostTurnForwardCommitMs);
    LogInfo << "Post-turn forward commit armed." << VAR(reason)
            << VAR(delta_degrees);
}
...
bool MotionController::IsForwardCommitActive(
    const std::chrono::steady_clock::time_point& now) const
{
    return turn_forward_commit_until_.time_since_epoch().count() > 0
        && now < turn_forward_commit_until_;
}
...
void MotionController::EnableAdaptiveMode(const char* reason, double metric)
{
    ...
    turn_scale_.units_per_degree = action_wrapper_->DefaultTurnUnitsPerDegree();
    turn_scale_.accepted_samples = std::max(
        turn_scale_.accepted_samples,
        kTurnBootstrapTargetSamples);
    const double units_per_degree = turn_scale_.units_per_degree;

    LogWarn << "Adaptive turn calibration enabled."
            << VAR(reason) << VAR(metric) << VAR(units_per_degree);
}
    ...
    int settle_wait_ms)
{
    ...
    const double predicted_turned_degrees =
        turn_scale_.PredictDegreesFromUnits(actuation.units_sent);
    session_->SyncVirtualYaw(
        NaviMath::NormalizeAngle(session_->virtual_yaw() + predicted_turned_degrees),
        "turn_predict");
    ...
    LogInfo << "Injected turn."
            << VAR(delta_degrees) << VAR(units_sent)
            << VAR(predicted_turned_degrees)
            << VAR(predicted_units_per_degree)
            << VAR(virtual_yaw);
    ...
    const bool sample_available = CaptureSettledTurnFeedback(
        &after_pos,
        expected_zone_id,
        requires_forward_feedback);
    double observed_turned_degrees = 0.0;
    bool same_direction = true;
    bool suspicious_sample = false;

    if (sample_available) {
        observed_turned_degrees =
            NaviMath::NormalizeAngle(after_pos.angle - yaw_before);
        same_direction = observed_turned_degrees == 0.0
            || ((observed_turned_degrees > 0.0) == (actuation.units_sent > 0));

        *position_ = after_pos;
        suspicious_sample = IsTurnSampleSuspicious(
            delta_degrees,
            actuation.units_sent,
            sample_available,
            observed_turned_degrees);
    ...
    if (suspicious_sample) {
        LogWarn << "Suspicious turn sample, switching to probe mode."
                << VAR(delta_degrees) << VAR(units_sent)
        ...
        if (turn_scale_.NeedsBootstrap()) {
            const bool pending_turn_may_not_have_applied =
                !sample_available
                || std::abs(observed_turned_degrees) < kTurnProbeMinObservedDegrees;
            RunTurnProbe(
                target_yaw,
                expected_zone_id,
                pending_turn_may_not_have_applied ? actuation.units_sent : 0,
                yaw_before);
        }
        return true;
    }
    ...
    if (!sample_available) {
        LogWarn << "Turn learning skipped, post-turn locate failed."
                << VAR(expected_zone_id);
        return true;
    }

    if (!same_direction) {
        LogWarn << "Turn learning skipped, observed turn direction mismatched."
                << VAR(observed_turned_degrees) << VAR(units_sent);
        return true;
    }

    const double previous_units_per_degree = turn_scale_.units_per_degree;
    if (turn_scale_.UpdateFromSample(
            actuation.units_sent,
            observed_turned_degrees)) {
        ...
        LogInfo << "Turn scale updated."
                << VAR(previous_units_per_degree)
                << VAR(updated_units_per_degree)
                << VAR(observed_turned_degrees)
                << VAR(units_sent)
                << VAR(accepted_samples);
    }
...
{
    return action == LocalDriverAction::Forward
        || action == LocalDriverAction::ForwardLeft
        || action == LocalDriverAction::ForwardRight
        || action == LocalDriverAction::JumpForward;
}

bool MotionController::ShouldLearnTurn(
    double delta_degrees,
    bool allow_learning) const
{
    const double abs_delta_degrees = std::abs(delta_degrees);
    if (!adaptive_mode_enabled_) {
        return false;
    }
    return allow_learning && !session_->is_waiting_for_zone_switch()
        && abs_delta_degrees >= kTurnLearningMinCommandDegrees
        && abs_delta_degrees <= kTurnLearningMaxCommandDegrees
        && abs_delta_degrees >= kTurnContinuousLearningMinDegrees;
}
...
bool MotionController::CaptureSettledTurnFeedback(
    ...
    while (!should_stop_()) {
        ...
        if (!position_provider_->Capture(&candidate_pos, false, expected_zone_id)) {
            ...
            if (waited_ms > kTurnFeedbackTimeoutMs) {
                ...
                if (has_any_pos) {
                    LogWarn << "Turn feedback settle timeout, use latest sample."
                            << VAR(expected_zone_id) << VAR(waited_ms);
                    return true;
                }
                return false;
            }
        ...
        *out_pos = candidate_pos;
        ...
        const bool is_angle_stable = has_last_pos
            && candidate_pos.zone_id == last_pos.zone_id
            && std::abs(NaviMath::NormalizeAngle(candidate_pos.angle - last_pos.angle))
                <= kTurnFeedbackStableAngleDegrees;
    ...
    double observed_delta_degrees) const
{
    ...
    const double trigger_min_degrees = bootstrap_learning
        ? kTurnBootstrapTriggerMinDegrees
        : kTurnProbeTriggerMinDegrees;
    ...
    const bool same_direction = observed_delta_degrees == 0.0
        || ((observed_delta_degrees > 0.0) == (units_sent > 0));
    if (!same_direction) {
        return true;
    }

    const double predicted_delta_degrees =
        std::abs(turn_scale_.PredictDegreesFromUnits(units_sent));
    const double allowed_error = bootstrap_learning
        ? std::max(
              kTurnBootstrapResidualDegrees,
              predicted_delta_degrees * kTurnBootstrapResidualRatio)
        : std::max(
              kTurnProbeResidualDegrees,
              predicted_delta_degrees * kTurnProbeResidualRatio);
    if (std::abs(abs_observed_delta_degrees - predicted_delta_degrees)
        > allowed_error) {
        return true;
    }
    ...
    if (!bootstrap_learning
        && abs_observed_delta_degrees
            > predicted_delta_degrees
                + std::max(
                    kTurnProbeOvershootResidualDegrees,
                    predicted_delta_degrees * kTurnProbeOvershootResidualRatio)) {
        return true;
    }
...
bool MotionController::RunTurnProbe(
    double target_yaw,
    const std::string& expected_zone_id,
    int pending_units,
    double pending_yaw_before)
{
    ...
    LogWarn << "Entering turn probe mode."
            << VAR(target_yaw) << VAR(expected_zone_id) << VAR(pending_units);
    ...
    for (int cycle = 0; cycle < kTurnProbeMaxCycles && !should_stop_(); ++cycle) {
        ...
        if (pending_sample) {
            ...
            const double remaining_delta_degrees =
                NaviMath::NormalizeAngle(target_yaw - session_->virtual_yaw());
            if (std::abs(remaining_delta_degrees) <= kTurnProbeSuccessDegrees) {
                return true;
            }

            const double probe_delta_degrees = std::clamp(
                remaining_delta_degrees,
                -kTurnProbeMaxDegreesPerCycle,
                kTurnProbeMaxDegreesPerCycle);
            ...
            const double predicted_probe_delta_degrees =
                turn_scale_.PredictDegreesFromUnits(cycle_units);
            session_->SyncVirtualYaw(
                NaviMath::NormalizeAngle(session_->virtual_yaw() + predicted_probe_delta_degrees),
                "turn_probe_predict");
            const double virtual_yaw = session_->virtual_yaw();

            LogInfo << "Turn probe cycle."
                    << VAR(cycle) << VAR(probe_delta_degrees)
                    << VAR(cycle_units) << VAR(predicted_probe_delta_degrees)
                    << VAR(virtual_yaw);
        }

        NaviPosition probe_pos;
        if (!CaptureSettledTurnFeedback(&probe_pos, expected_zone_id, true)) {
            LogWarn << "Turn probe locate failed."
                    << VAR(cycle) << VAR(expected_zone_id);
            continue;
        }

        const int sample_units = cycle_units != 0 ? cycle_units : pending_units;
        const double observed_delta_degrees =
            NaviMath::NormalizeAngle(probe_pos.angle - yaw_before_cycle);
        const bool same_direction = observed_delta_degrees == 0.0
            || ((observed_delta_degrees > 0.0) == (sample_units > 0));
        ...
        *position_ = probe_pos;
        ...
        if (sample_units != 0 && same_direction) {
            ...
            if (turn_scale_.UpdateFromSample(
                    sample_units,
                    observed_delta_degrees)) {
                const double units_per_degree = turn_scale_.units_per_degree;
                const int accepted_samples = turn_scale_.accepted_samples;
                LogInfo << "Turn scale updated from probe."
                        << VAR(previous_units_per_degree)
                        << VAR(units_per_degree)
                        << VAR(observed_delta_degrees)
                        << VAR(sample_units)
                        << VAR(accepted_samples);
            }
        }
