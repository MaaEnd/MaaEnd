...
bool NaviController::Navigate(const NaviParam& param)
{
    ...
    LogInfo << "MapNavigator controller initialized."
            << VAR(controller_type)
            << VAR(uses_touch_backend);
    if (!action_wrapper.is_supported()) {
        const char* unsupported_reason = action_wrapper.unsupported_reason();
        LogError << "MapNavigator controller backend is unsupported."
                 << VAR(controller_type)
    ...
    NaviPosition pos;
    const std::string initial_expected_zone =
        param.path.front().zone_id.empty() ? param.map_name : param.path.front().zone_id;
    if (!position_provider.WaitForFix(
            &pos,
            initial_expected_zone,
            kLocatorWaitMaxRetries,
            kLocatorWaitIntervalMs,
            is_stopping)
        || is_stopping()) {
        return false;
    }
    ...
    RouteRejoinPlanner rejoin_planner(
        param.rejoin_abort_distance,
        param.rejoin_candidate_limit,
        param.rejoin_backtrack_window);
