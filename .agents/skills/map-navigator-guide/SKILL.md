...
角色箭头方向与下一步目标方向差出 30° 以上时，停步做一次**预对齐**：把镜头转到下一步目标方向，对齐后再恢复前进。其余转向照旧在移动中完成。预对齐只在能拿到相机朝向时做——拿不到就不要停步转向；发不出去或对不上就判失败，直接回退到移动中转向。对齐后恢复走路，要等角色朝向追进死区（上限 800 ms）才恢复发转向指令。
