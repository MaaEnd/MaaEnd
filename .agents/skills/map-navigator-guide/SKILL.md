...
**只有框、没有坐标的目标用 `FIND`，别在 Pipeline 里拿 `NeuralNetworkDetect` + `CharacterMoveToTargetAction` 自己搓。** `FIND` 点用 `find_target`（点名一个出框的节点）或 `find_text`（内联 OCR 文本表）说清找什么，用 `find_stop`（命中节点）或 `find_arrive`（`[x, y]` 走到附近）说清什么算到位，两者至少给一个；写了 `find_arrive` 的点每拍读一次定位算距离（不参与转向），其余时刻不读小地图，剩下的转视角搜索、按框前进、走过头退一步全由导航器做。带 `target` 时先走到锚点再找，不带就是就地开找的控制节点。转视角是开环发的，它是定位的断层：退出时作废操舵与走廊记账，所以后面要接普通移动点而不是直接收尾。
