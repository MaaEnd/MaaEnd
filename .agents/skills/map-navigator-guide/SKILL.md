...
只有启用滑索时，`path` 中用于移动或可达性兜底的节点才作为可选路线 / 动作提示参与全局优化：运行时先尝试从区间起点直达末端位置，成功时跳过中间的 `NAVMESH`、录制 `RUN`、`SPRINT`、`JUMP`、`FIGHT`、`TRANSFER`、`PORTAL`、`INTERACT` 等点及其动作，失败才按完整作者路径逐段执行。确实必须抵达并执行的节点写 `required: true`，它会切断区间并成为后续规划的新起点。跨越多张面的长距离仍可逐段给提示点，用它们为直达失败提供可达性兜底。

未启用滑索时不做跨作者路径点的全局比较，也不跳过任何动作：普通点按作者顺序保留，`NAVMESH` 只逐点展开，显式 `target_tier` 仍按原规则投影。

`ZONE` 是结构边界；`HEADING`、`COLLECT`、`DIG` 是固有必执行边界，不需要 `required: true`。`HEADING` 是明确控制指令；`COLLECT` 决定行进采集扫描覆盖的区域；`DIG` 到点派发挖掘子任务。即使启用滑索，全局展开也不能删除这三类节点。
